...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    if (kind === 'kpi_18_6_meta') {
      // Penandatangan KPI 18.6 Jadwal PM Terkendali -- tabel sendiri
      // (kpi_18_6_meta), label "Mengetahui/ Menyetujui" (lihat kpi.js).
      await ensureKpiTables();
      await saveKpi18_6Meta(req.body);
      return res.status(200).json({ success: true });
    }
    if (kind === 'kpi_activity_plan_row') {
      // Simpan satu baris Activity Plan (item/grup): kolom manual + nilai %
      // manual (untuk item manual). Lihat kpi.js.
      await ensureKpiTables();
    ...
    if (kind === 'kpi_jadwal_kegiatan_meta') {
      // Keterangan tiap baris kegiatan, pejabat penandatangan (3 kolom), dan
      // kode dokumen footer Jadwal Kegiatan -- disimpan global (period_key
      // 'global'), lihat kpi.js.
      await ensureKpiTables();

(function () {
  ...
  var state = null;      // respons terakhir dari /api/visualization/data?dataType=kpi_18_6
  var currentPeriod = 0; // 0 = Jan-Jun tahun, 1 = Jul-Des tahun sebelumnya
  var isAdmin = false;
  ...
  async function fetchApiData(tahun) {
    ...
    var url = '/api/visualization/data?dataType=kpi_18_6' + (tahun ? ('&tahun=' + encodeURIComponent(tahun)) : '');
  ...
  // ------------------------------------------------------------------
  // RENDER TABEL (Rencana & Realisasi per bulan)
  // ------------------------------------------------------------------
  function buildHead(period) {
    var idxs = monthIndexesFor(period);
    var html = '<tr><th rowspan="2" style="min-width:30px;">NO</th><th rowspan="2" style="min-width:200px;">URAIAN</th><th rowspan="2" style="min-width:52px;">Renc/<br>Real</th>';
    idxs.forEach(function (mi) { html += '<th style="min-width:120px;">' + monthLabel(mi) + '</th>'; });
    html += '</tr>';
    return html;
  }
  ...
  function renderTable() {
    ...
    state.groups.forEach(function (g) { html += itemRows(g, idxs); });
    html += '</tbody>';
  ...
  // ------------------------------------------------------------------
  // PENANDATANGAN (global, pola sama KPI lain)
  // ------------------------------------------------------------------
  ...
  document.getElementById("downloadBtn").addEventListener("click", async function () {
    if (!isAdmin) {
      ...
      window.location.href = '../../login.html?redirect=' + encodeURIComponent('apps/kpi-sab/jadwal-pm.html');
      return;
    }
    var btn = document.getElementById('downloadBtn');
    ...
    try {
      ...
      var apiUrl = '/api/visualization/data?dataType=kpi_18_6_xlsx&tahun=' + encodeURIComponent(state.tahun) +
      ...
      a.href = url; a.download = '18.6 Jadwal PM Terkendali ' + state.tahun + '.xlsx';
  ...
  function openWhatsappChat() {
    ...
    var message = 'Halo, saya' + namaPart + ' baru saja mengirim permintaan akses data KPI 18.6 Jadwal PM Terkendali di website, mohon persetujuannya.';

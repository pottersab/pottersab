(function () {
  ...
  var state = null;      // respons terakhir dari /api/visualization/data?dataType=kpi_apatd
  ...
  // (ukur-debit), supaya sekali disetujui admin, berlaku juga di halaman ini. ---
  ...
  async function fetchApiData(tahun) {
    ...
    var url = '/api/visualization/data?dataType=kpi_apatd' + (tahun ? ('&tahun=' + encodeURIComponent(tahun)) : '');
  ...
  function buildHead(period) {
    ...
    var html = '<tr><th style="min-width:34px;">NO</th><th style="min-width:190px;">INSTALASI PENGOLAHAN</th>';
    idxs.forEach(function (mi) { html += '<th class="month">' + MONTHS[mi] + '<br><span style="font-weight:500;font-size:9px;text-transform:none;">(m&sup3;)</span></th>'; });
    return html + '</tr>';
  }
  ...
  function renderGroup(html, group, idxs) {
    ...
    html += '<tr class="grouphead"><td colspan="' + (span + 1) + '" class="tag">' + group.label + ' <span class="name-flag">Data Air Baku</span></td></tr>';

    var sums = {}; idxs.forEach(function (mi) { sums[mi] = 0; });

    group.items.forEach(function (item, i) {
      html += '<tr><td class="no">' + (i + 1) + '</td><td class="name">' + item.label + '</td>';
      idxs.forEach(function (mi) {
        if (item.values[mi] !== null) sums[mi] += item.values[mi];
        html += cellVal(item.values[mi]);
      });
      html += '</tr>';
    });

    html += '<tr class="sum"><td></td><td>JUMLAH</td>';
    idxs.forEach(function (mi) {
      var anyData = group.items.some(function (item) { return item.values[mi] !== null; });
      html += '<td><div class="cellwrap" style="justify-content:flex-end;">' + (anyData ? fmt(sums[mi]) : '') + '</div></td>';
    });
    html += '</tr>';
    return html;
  }
  ...
  function renderTable() {
    ...
    document.getElementById("tableFoot").innerHTML =
      'Sumber: <b>Data &amp; Visualisasi → Data Pengambilan Air Baku → Air Permukaan (AP) / Air Tanah Dalam (ATD)</b> · ' +
      'Format unduhan mengikuti <b>18.3A APATD.xlsx</b> (dua blok 6 bulan per instalasi, JUMLAH per kelompok).';
  }

  function renderStats() {
    var idxs = monthIndexesFor(currentPeriod);
    var totalInst = 0;
    state.groups.forEach(function (g) { totalInst += g.items.length; });

    // Total debit bulan TERAKHIR yang sudah ada datanya di periode yang
    // sedang ditampilkan, per kelompok (AP/ATD) dan gabungan.
    var lastMonth = null, totals = {};
    state.groups.forEach(function (g) { totals[g.key] = 0; });
    for (var k = idxs.length - 1; k >= 0; k--) {
      var mi = idxs[k];
      var any = state.groups.some(function (g) { return g.items.some(function (item) { return item.values[mi] !== null; }); });
      if (any) {
        lastMonth = mi;
        state.groups.forEach(function (g) {
          g.items.forEach(function (item) { if (item.values[mi] !== null) totals[g.key] += item.values[mi]; });
        });
        break;
      }
    }
    var grandTotal = state.groups.reduce(function (s, g) { return s + (totals[g.key] || 0); }, 0);

    var stat = document.getElementById("statRow");
    var html =
      '<div class="stat"><div class="k">Instalasi</div><div class="v">' + totalInst + '</div></div>';
    state.groups.forEach(function (g) {
      html += '<div class="stat"><div class="k">' + g.label + (lastMonth !== null ? ' (' + MONTHS[lastMonth] + ')' : '') + '</div><div class="v">' +
        (lastMonth !== null ? fmt(totals[g.key]) + ' m³' : '–') + '</div></div>';
    });
    html += '<div class="stat"><div class="k">Total AP + ATD' + (lastMonth !== null ? ' (' + MONTHS[lastMonth] + ')' : '') + '</div><div class="v good">' +
      (lastMonth !== null ? fmt(grandTotal) + ' m³' : '–') + '</div></div>';
    stat.innerHTML = html;
  }
  ...
  function renderStatusBadge() {
    ...
    var filled = 0;
    state.groups.forEach(function (g) { g.items.forEach(function (item) { item.values.forEach(function (v) { if (v !== null) filled++; }); }); });
  ...
  document.getElementById("downloadBtn").addEventListener("click", async function () {
    if (!isAdmin) {
      ...
      window.location.href = '../../login.html?redirect=' + encodeURIComponent('apps/kpi-sab/apatd.html');
      return;
    }
    var btn = document.getElementById('downloadBtn');
    ...
    try {
      ...
      var apiUrl = '/api/visualization/data?dataType=kpi_apatd_xlsx&tahun=' + encodeURIComponent(state.year) +
      ...
      a.href = url; a.download = '18.3A APATD ' + state.year + '.xlsx';
  ...
  function openWhatsappChat() {
    ...
    var message = 'Halo, saya' + namaPart + ' baru saja mengirim permintaan akses data KPI 18.3a APATD di website, mohon persetujuannya.';
  ...
  async function submitAccessRequest() {
    ...
    try {
      var res = await fetch('/api/visualization/request', {
        ...
        body: JSON.stringify({ requestedBy: nama, dataType: 'kpi_apatd', reason: alasan || undefined })
      });

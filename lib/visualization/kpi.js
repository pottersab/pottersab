...
function buildK97Sheet(wb, cfg, ipa, data) {
  ...
  rows.forEach((row, i) => {
    ...
    setv(4, r, row.levelLaluStatis == null ? '-' : row.levelLaluStatis, cal11); ctr(CELL(4, r));
    setv(5, r, row.levelLaluDinamis == null ? '-' : row.levelLaluDinamis, cal11); ctr(CELL(5, r));
    setv(6, r, row.levelIniStatis == null ? '-' : row.levelIniStatis, cal11); ctr(CELL(6, r));
    setv(7, r, row.levelIniDinamis == null ? '-' : row.levelIniDinamis, cal11); ctr(CELL(7, r));
    ...
    setv(9, r, row.realLalu == null ? '-' : row.realLalu, cal11); ctr(CELL(9, r));
    setv(10, r, row.realIni == null ? '-' : row.realIni, cal11); ctr(CELL(10, r));
    MRG(11, r, colCount, r);
    const kc = setv(11, r, row.keterangan, cal11);
    kc.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
  });
  ...
  if (rows.length) {
    const sumNum = (key) => rows.reduce((s, r) => { const v = Number(r[key]); return s + (Number.isFinite(v) ? v : 0); }, 0);
    setv(8, tr, { formula: `SUM(H${dataTop}:H${last})`, result: sumNum('namePlate') }, ar10b); ctr(CELL(8, tr));
    setv(9, tr, { formula: `SUM(I${dataTop}:I${last})`, result: sumNum('realLalu') }, ar10b); ctr(CELL(9, tr));
    setv(10, tr, { formula: `SUM(J${dataTop}:J${last})`, result: sumNum('realIni') }, ar10b); ctr(CELL(10, tr));
  }
  for (let c = 1; c <= colCount; c++) {

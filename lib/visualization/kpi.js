...
function buildKpiActivityPlanExcelWorkbook(data) {
  ...
  // Kop (Arial 12 tebal) + "Form Revisi 1" + tanggal
  ws.mergeCells('C1:D1'); apCell(ws, 'C', 1, 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG', { name: 'Arial', size: 12, bold: true }).alignment = { horizontal: 'left' };
  ws.mergeCells('C2:D2'); apCell(ws, 'C', 2, 'KOTA BALIKPAPAN', { name: 'Arial', size: 12, bold: true }).alignment = { horizontal: 'left' };
  apCell(ws, 'U', 2, 'Form Revisi 1', AP_CAL11).alignment = { horizontal: 'right' };
  apCell(ws, 'R', 3, 'Balikpapan, ' + (data.periode || ''), AP_CAL11);
  ...
  ws.mergeCells('B5:P5'); apCell(ws, 'B', 5, 'PERIODE: ' + (data.periode || ''), AP_CAL11).alignment = { horizontal: 'left' };
  ws.mergeCells('B6:P6'); apCell(ws, 'B', 6, 'BAGIAN : PRODUKSI', AP_CAL11).alignment = { horizontal: 'left' };
  ws.mergeCells('B7:D8'); apCell(ws, 'B', 7, 'SUB BAGIAN SUMBER AIR DAN LINGKUNGAN', AP_CAL11).alignment = { horizontal: 'left', vertical: 'middle' };
  ws.mergeCells('R8:S8'); apCell(ws, 'R', 8, 'ALI RACHMAN AS', AP_CAL11);
  apCell(ws, 'T', 8, 'DEDY HERMAWAN', AP_CAL11);
  apCell(ws, 'U', 8, 'DARTO', AP_CAL11);
  ...
  for (let rr = H1; rr <= H2; rr++) {
    for (let c = 2; c <= 21; c++) {
      const cell = ws.getRow(rr).getCell(c);
      if (rr === H2 && !cell.font) cell.font = AP_CAL11;
      cell.border = GRID;
    }
  }
  ...
  // Seksi "IV / Sub Bagian Sumber Air dan Lingkungan"
  ws.mergeCells('B12:B13'); apCell(ws, 'B', 12, 'IV', AP_TAH12B);
  ws.mergeCells('E12:E13'); apCell(ws, 'E', 12, 'Sub Bagian Sumber Air dan Lingkungan', AP_TAH12B).alignment = { horizontal: 'left' };
  ...
  // Tanda tangan (sederhana, mengikuti gaya KPI lain)
  r += 1;
  ws.mergeCells(`Q${r}:R${r}`); apCell(ws, 'Q', r, 'Mengetahui/ Menyetujui', AP_CAL11);
  ws.mergeCells(`S${r}:U${r}`); apCell(ws, 'S', r, 'Balikpapan, 7 Juli 2026', AP_CAL11);
  r++;
  ws.mergeCells(`Q${r}:R${r}`); apCell(ws, 'Q', r, 'Manajer Produksi', AP_CAL11);
  r += 4;
  ws.mergeCells(`Q${r}:R${r}`); apCell(ws, 'Q', r, 'DEDY HERMAWAN, S.M', { name: 'Calibri', size: 12, bold: true });
  ws.mergeCells(`S${r}:U${r}`); apCell(ws, 'S', r, 'DARTO', { name: 'Calibri', size: 12, bold: true });

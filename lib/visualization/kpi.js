...
async function getKpiActivityPlanData(access, tahunQuery) {
  ...
  return { locked: !granted, availableYears: [], tahun, periode: 'JANUARI - DESEMBER ' + tahun, signPlaceDate: todaySignDate(), groups };
}
...
function buildKpiActivityPlanExcelWorkbook(data) {
  ...
  // Tanggal bisa diisi manual dari halaman (data.signPlaceDate), merge lebar
  // di kanan atas supaya teks tanggal yang panjang muat.
  ws.mergeCells('R3:U3');
  ...
  // PERIODE / DIVISI / SUB BAGIAN (center) + border utuh di area atas supaya
  ...
  ws.mergeCells('B7:D8'); apCell(ws, 'B', 7, 'SUB BAGIAN SUMBER AIR DAN LINGKUNGAN', AP_CAL11).alignment = { horizontal: 'center', vertical: 'middle' };
  ...
  for (let rr = 1; rr <= 8; rr++) {
    for (let c = 2; c <= 21; c++) ws.getRow(rr).getCell(c).border = GRID;
  }

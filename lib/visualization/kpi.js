...
// Indeks fiskal 0-11 untuk 18.6 = bulan KALENDER (0=Januari..11=Desember),
// cuma TAHUN-nya yang beda: 0-5 = Januari..Juni tahun terpilih, 6-11 =
// Juli..Desember tahun SEBELUMNYA.
function kpi18_6Calendar(tahun, mi) {
  return { year: mi < 6 ? Number(tahun) : Number(tahun) - 1, monthIndex: mi };
}
...
function kpi18_6MonthTitle(tahun, mi) {
  ...
  return `${MONTHS_TITLE[monthIndex]} ${year}`;
}
...
function buildKpi18_6Block(ws, rowOffset, monthIdx, judulBulan, data, meta, tahun) {
  ...
  // Data: tiap kelompok -> baris Rencana + baris Realisasi per item. NO di
  // merge vertikal sepanjang kelompok; URAIAN di merge B:D per baris.
  let r = R(7);
  data.groups.forEach(g => {
    const firstRow = r;
    const totalRows = g.items.length * 2;
    ...
    g.items.forEach(it => {
      [[it.renc, 'Renc', it.rencDates], [it.real, 'Real', it.realDates]].forEach(([label, kind, dates]) => {
        ...
        // Cuma tulis 6 bulan milik blok ini (monthIdx), ke kolom F-K.
        monthIdx.forEach((mi, i) => {
          const c = colLetter(6 + i);
          const cell = ws.getCell(`${c}${r}`);
          cell.value = dates[mi];
    ...
    for (let rr = firstRow; rr <= r - 1; rr++) {
      for (let c = 2; c <= 11; c++) ws.getRow(rr).getCell(c).border = GRID;
    }
  });
  const lastDataRow = r - 1;
...
async function buildKpi18_6ExcelWorkbook(data) {
  ...
  buildKpi18_6Block(ws, nextRow, [6, 7, 8, 9, 10, 11], `BULAN : Juli - Desember ${Number(data.tahun) - 1}`, data, meta, data.tahun);

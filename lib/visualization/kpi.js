...
function buildKpi9_8ExcelWorkbook(data) {
  ...
  // Bagian / Lokasi / Bulan (nilai di-merge B:F, kolom G terpisah -- contoh)
  ...
  for (let c = 1; c <= 7; c++) bdT(c, 9, 't');
  for (let c = 1; c <= 7; c++) bdT(c, 11, 'b');
  for (let r = 9; r <= 11; r++) { bdT(1, r, 'l'); bdT(7, r, 'r'); }
  ...
  // Border header: THIN atas/tepi, bawah baris 13 HAIR (pola contoh).
  const setHb = (c, r, sides) => {
    const b = CELL(c, r).border || {};
    ['top', 'left', 'bottom', 'right'].forEach(s => { if (sides.indexOf(s[0]) >= 0) b[s] = THIN; });
    CELL(c, r).border = b;
  };
  setHb(1, 12, 'tlr'); setHb(2, 12, 'tlr'); setHb(3, 12, 'tlr'); setHb(4, 12, 'tlr');
  setHb(5, 12, 'tlbr'); // KALIBRASI EXTERNAL kotak penuh
  setHb(7, 12, 'tlr');
  setHb(5, 13, 'lr'); setHb(6, 13, 'lr'); // TERAKHIR / BERIKUTNYA

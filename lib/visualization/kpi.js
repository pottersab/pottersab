...
const { fetchSumurWells, fetchSumurDebitRows, fetchSumurLevelRows, fetchRealRows, fetchWideSingleRows } = require('./repo');
const { buildDummySumurDebitRows, buildDummySumurLevelRows, buildDummyRows, buildDummyWideSingleRows } = require('./dummy');
...
function buildPengambilanExcelBlock(ws, rowOffset, monthNames, monthBase, judulBulan, data) {
  ...
  data.groups.forEach((g, gi) => {
    ...
    monthNames.forEach((mn, i) => {
      ...
      const hasReal = realVal !== null && realVal !== undefined;
      const anggCell = ws.getCell(`${ac}${r}`);
      const realCell = ws.getCell(`${rc}${r}`);
      if (hasReal) {
        // Bulan ini data real sudah terinput: tampilkan anggar, real, ratio.
        if (anggVal !== null && anggVal !== undefined) anggCell.value = anggVal;
        realCell.value = realVal;
        ws.getCell(`${pmc}${r}`).value = { formula: `IF(OR(${rc}${r}="",${ac}${r}=""),"",${rc}${r}-${ac}${r})` };
        ws.getCell(`${pctc}${r}`).value = { formula: `IF(OR(${rc}${r}="",${ac}${r}=""),"",${rc}${r}/${ac}${r}*100)` };
        anggCell.numFmt = '#,##0';
        realCell.numFmt = '#,##0';
        ws.getCell(`${pmc}${r}`).numFmt = '#,##0';
        // Ratio Efisiensi % ditampilkan 2 angka di belakang koma (revisi user).
        ws.getCell(`${pctc}${r}`).numFmt = '#,##0.00';
      } else {
        // Data real belum diinput: anggar, real, & ratio bulan ini disembunyikan
        // dulu (revisi user) -- ketiga sel dibiarkan kosong.
        anggCell.value = null;
        realCell.value = null;
      }
    });
  ...
  for (let rr = R(10); rr <= r - 1; rr++) {
    for (let c = 1; c <= 26; c++) {
      const cell = ws.getRow(rr).getCell(c);
      // Font data A33-Z34 (juga A10-Z11) jadi Times New Roman 14 -- revisi user.
      if (!cell.font) cell.font = { size: 14, name: TNR };
      cell.alignment = cell.alignment || { horizontal: 'center' };
      cell.border = GRID;
    }
  }
...
// Bulan terakhir ('YYYY-MM') yang ADA datanya di tabel sumber -- dipakai untuk
// DEFAULT pemilih bulan laporan bulanan (pola 19.2): kalau bulan berjalan belum
// berisi data, laporan otomatis terbuka di bulan terakhir yang terisi, bukan
// bulan berjalan yang mungkin masih kosong. specs: [{ table, dateCol }] atau
// [{ table, monthCol }].
async function latestDataYm(specs) {
  const parts = (specs || []).map(s => `SELECT to_char(${s.monthCol || s.dateCol}, 'YYYY-MM') AS m FROM ${s.table}`);
  if (!parts.length) return null;
  const sql = `SELECT MAX(x.m) AS m FROM (${parts.join(' UNION ALL ')}) x`;
  const { rows } = await pool.query(sql);
  return rows[0] && rows[0].m ? rows[0].m : null;
}
...
async function getKpiKualitasData(access, bulanQuery) {
  ...
  // Default: bulan terakhir yang ADA datanya (kalau ada & dalam rentang laporan),
  // supaya laporan tidak terbuka di bulan berjalan yang masih kosong.
  if (!bulan) {
    const latest = await latestDataYm([
      { table: 'manggar_level_curahhujan', dateCol: 'tanggal' },
      { table: 'kualitas_air_manggar_teritip', dateCol: 'tanggal' },
      { table: 'teritip_level', dateCol: 'tanggal' }
    ]);
    if (latest && ymToIndex(latest) >= ymToIndex(KUALITAS_START_YM)) bulan = latest;
  }
  ...
  const monthStart = `${year}-${pad2(monthIndex + 1)}-01`;
  const fetched = {};
  // Hanya satu bulan yang dibutuhkan laporan ini (wideSingleMonthMap di bawah
  // menyaringnya), jadi fetch dibatasi ke rentang bulan itu -- bukan seluruh
  // ribuan baris harian sejak 2014. Kalau tabel 4 key yang sama
  // (kualitas_air_manggar_teritip) tetap di-fetch per key, tapi tiap query
  // kini hanya membawa baris sebulan.
  await Promise.all(dsKeys.map(async k => {
    fetched[k] = await fetchWideSingleRows(DATASETS[k], { start: monthStart, end: monthEnd });
  }));
...
// Kolom A-G lebar 11 (82px) -- revisi user; H (KETERANGAN) tetap 22.
const KUAL_COL_WIDTHS = [11, 11, 11, 11, 11, 11, 11, 22];
...
const KPI_18_1B_SECTIONS = [
  ...
  // Kampung Damai: no 4 = Bukit Cinta (sudah nonaktif, jadi Jl Penggalang
  // tetap tampil sebagai no 5). Zamp: no 2 = Jl Belibis 5, no 3 = Koperasi PTMB
  // (revisi label 18.1B).
  { installation: 'kampung_damai', label: 'II. KAMPUNG DAMAI', names: ['Parkiran IPA', 'Belakang Gas chlor', 'Terminal Tangki', 'Bukit Cinta', 'Jl Penggalang'] },
  { installation: 'prapatan', label: 'III. PRAPATAN', names: ['Samping PUSKESMAS', 'Inst Prapatan', 'Jl Pahala'] },
  { installation: 'zamp', label: 'IV. ZAMP', names: ['Jalan Belibis', 'Jl Belibis 5', 'Koperasi PTMB'] },
...
async function getKpiLevelStatisDinamisData(access, tahunQuery) {
  ...
  const groups = KPI_18_1B_SECTIONS.map((section, si) => {
    ...
    allWells.forEach((w, idx) => {
      ...
      no++;
      wells.push({
        // Nomor urut memakai NOMOR SUMUR dari nama DB (mis. "Sumur_05_Penggalang"
        // -> 5) supaya sumur yang nonaktif (mis. no 4 Bukit Cinta) TIDAK menggeser
        // penomoran sumur aktif lain -- Jl Penggalang tetap tampil no 5, bukan 4.
        // Fallback ke urutan aktif bila nama tidak memuat nomor.
        no: k97WellNo(w.well_name, no),
...
function buildKpi18_5Block(ws, base, monthIdx, judulBulan, data, meta, tahun) {
  ...
  data.groups.forEach(g => {
    ...
    ws.getRow(r).height = 37.5; // 50px (revisi user) -- 1px = 0.75pt
    r++;
  });
...
//
// Untuk source 'pengambilan_ap'/'pengambilan_atd' data laporannya SAMA
// (getKpiPengambilanData mengembalikan dua-duanya sekaligus), jadi datanya
// boleh di-pass lewat param `pengambilanData` supaya tidak di-fetch dua kali
// oleh pemanggil yang butuh kedua-duanya (getKpiActivityPlanData).
async function activityAutoPct(source, tahun, periode, pengambilanData) {
  ...
  const data = pengambilanData || await getKpiPengambilanData({ granted: true, kind: 'admin' }, tahun);
...
async function getKpiActivityPlanData(access, tahunQuery, periodeQuery) {
  ...
  const auto = {};
  if (granted) {
    // getKpiPengambilanData di-fetch SEKALI lalu datanya dipakai untuk
    // pengambilan_ap DAN pengambilan_atd (sebelumnya di-fetch dua kali).
    // Panggilan yang datanya independen (level/debit) dijalankan paralel.
    const [level, debit, pengambilanData] = await Promise.all([
      activityAutoPct('level', tahun, periode),
      activityAutoPct('debit', tahun, periode),
      getKpiPengambilanData({ granted: true, kind: 'admin' }, tahun)
    ]);
    auto.level = level;
    auto.debit = debit;
    auto.pengambilan_ap = await activityAutoPct('pengambilan_ap', tahun, periode, pengambilanData);
    auto.pengambilan_atd = await activityAutoPct('pengambilan_atd', tahun, periode, pengambilanData);
  }
...
function apWriteRow(ws, r, row, isGroup) {
  ...
  // Revisi user: seluruh isi tabel data (B14-U37) font Times New Roman 14
  // (dulu Calibri 11). Baris grup tetap tebal, sub-item label tetap tebal.
  apCell(ws, 'F', r, row.target, isGroup ? AP_TNR14B : AP_TNR14);
  apCell(ws, 'G', r, row.timing, isGroup ? AP_TNR14B : AP_TNR14);
  AP_P.forEach((col, i) => {
    ...
    cell.font = isGroup ? AP_TNR14B : AP_TNR14;
    cell.alignment = { horizontal: 'center', vertical: 'middle' };
  });
  apCell(ws, 'N', r, row.status, isGroup ? AP_TNR14B : AP_TNR14);
  apCell(ws, 'O', r, row.trend, isGroup ? AP_TNR14B : AP_TNR14);
  apCell(ws, 'P', r, row.problem, isGroup ? AP_TNR14B : AP_TNR14);
  ws.mergeCells(`Q${r}:R${r}`);
  apCell(ws, 'Q', r, row.corrective, isGroup ? AP_TNR14B : AP_TNR14);
  apCell(ws, 'S', r, row.pic, isGroup ? AP_TNR14B : AP_TNR14);
  apCell(ws, 'T', r, row.due_date, isGroup ? AP_TNR14B : AP_TNR14);
  apCell(ws, 'U', r, row.progres, isGroup ? AP_TNR14B : AP_TNR14);
  for (let c = 2; c <= 21; c++) ws.getRow(r).getCell(c).border = GRID;
  ws.getRow(r).height = 48.75; // 65px (revisi user) -- baris data 14-37
  return r + 1;
}
...
function buildKpiActivityPlanExcelWorkbook(data) {
  ...
  // Nama pejabat (Approved/Checked/Prepared) dibuat tebal -- revisi user.
  ws.mergeCells('R8:S8'); apCell(ws, 'R', 8, officials.approved, AP_CAL11B);
  apCell(ws, 'T', 8, officials.checked, AP_CAL11B);
  apCell(ws, 'U', 8, officials.prepared, AP_CAL11B);
  ...
  // Tinggi baris 14-37 seragam 65px (48.75pt) -- revisi user (dulu baris grup
  // 18 sengaja lebih tinggi 66px supaya label panjang tidak terpotong).
  ws.getRow(14).height = 48.75;
...
async function getKpi9_2Data(access, bulanQuery) {
  const currentYm = currentYmWITA();
  let bulan = (typeof bulanQuery === 'string' && /^\d{4}-\d{2}$/.test(bulanQuery)) ? bulanQuery : null;
  // Default: bulan terakhir yang ADA datanya (sumber sama 18.4).
  if (!bulan) {
    const latest = await latestDataYm([
      { table: 'manggar_level_curahhujan', dateCol: 'tanggal' },
      { table: 'kualitas_air_manggar_teritip', dateCol: 'tanggal' },
  ...
  const monthStart = `${year}-${pad2(monthIndex + 1)}-01`;
  const fetched = {};
  // Sama seperti 18.4: fetch dibatasi ke satu bulan laporan, bukan seluruh
  // baris harian tabel.
  await Promise.all(dsKeys.map(async k => {
    fetched[k] = await fetchWideSingleRows(DATASETS[k], { start: monthStart, end: monthEnd });
  }));
...
async function getKpi9_3Data(access, bulanQuery) {
  const currentYm = currentYmWITA();
  let bulan = (typeof bulanQuery === 'string' && /^\d{4}-\d{2}$/.test(bulanQuery)) ? bulanQuery : null;
  // Default: bulan terakhir yang ADA datanya (Level/Curah Hujan Manggar).
  if (!bulan) {
    const latest = await latestDataYm([
      { table: 'manggar_level_curahhujan', dateCol: 'tanggal' }
    ]);
  ...
  // Fetch dibatasi ke satu bulan laporan, bukan seluruh baris harian tabel
  // (pola sama 18.4/9.2).
  const k93Start = `${year}-${pad2(monthIndex + 1)}-01`;
  const k93End = `${year}-${pad2(monthIndex + 1)}-${pad2(daysInMonth(year, monthIndex))}`;
  const [levelRows, hujanRows] = await Promise.all([
    fetchWideSingleRows(DATASETS.manggar_level, { start: k93Start, end: k93End }),
    fetchWideSingleRows(DATASETS.manggar_hujan, { start: k93Start, end: k93End })
  ]);
  const built = buildK93Rows(
...
function buildKpi9_3ExcelWorkbook(data) {
  ...
  // Right border I45-I48 -- kolom I bagian dari merge G:I, jadi border kanan
  // di-set di sel MASTER (G) supaya tepi kanan merge benar-benar tampil
  // (border pada slave merge diabaikan Excel).
  [45, 46, 47, 48].forEach(rr => bdT(7, rr, 'r'));
...
// 9.4 Laporan Ketidaksesuaian Debit (apps/kpi-sab/ketidaksesuaian-9-4.html).
//
// Semua isian MANUAL oleh admin (tidak ada data otomatis):
//   items[] -> per ketidaksesuaian: ds (debit sebelumnya), di (debit hari
//              ini), ket (keterangan). NO = urutan. Disimpan per bulan di
//              kpi_9_4_items (items JSONB).
//   meta    -> penandatangan 2 kolom (Menyetujui/Disetujui + Diketahui) &
//              kode dokumen "PTMBPP-IR-PRD.SAB/01-04" (kpi_9_4_meta, global).
// Unduhan Excel = persis contoh "9.4 Laporan Ketidaksesuaian debit.xlsx"
// (sheet "Ketidaksesuaian debit 04").
// ===========================================================================

const K94_META_KEY = 'global';
const K94_DEFAULT_META = {
  roleMenyetujui: 'Manajer Produksi',
  nameMenyetujui: 'DEDY HERMAWAN S.M',
  roleDiketahui: 'Supervisor Sumber Air Baku & Lingkungan',
  nameDiketahui: 'DARTO',
  footerCode: 'PTMBPP-IR-PRD.SAB/01-04'
};

async function loadGlobalK94Meta() {
  const { rows } = await pool.query('SELECT * FROM kpi_9_4_meta WHERE period_key = $1', [K94_META_KEY]);
  if (!rows[0]) return null;
  const r = rows[0];
  return {
    roleMenyetujui: r.role_menyetujui || '',
    nameMenyetujui: r.name_menyetujui || '',
    roleDiketahui: r.role_diketahui || '',
    nameDiketahui: r.name_diketahui || '',
    footerCode: r.footer_code || ''
  };
...
async function saveK94Meta(m) {
  await pool.query(
    `INSERT INTO kpi_9_4_meta (period_key, role_menyetujui, name_menyetujui, role_diketahui, name_diketahui, footer_code, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, now())
     ON CONFLICT (period_key) DO UPDATE SET
       role_menyetujui = EXCLUDED.role_menyetujui, name_menyetujui = EXCLUDED.name_menyetujui,
       role_diketahui = EXCLUDED.role_diketahui, name_diketahui = EXCLUDED.name_diketahui,
       footer_code = EXCLUDED.footer_code, updated_at = now()`,
    [K94_META_KEY, m.roleMenyetujui || '', m.nameMenyetujui || '', m.roleDiketahui || '', m.nameDiketahui || '', m.footerCode || '']
  );
}

async function loadK94Items(bulan) {
  const { rows } = await pool.query('SELECT items FROM kpi_9_4_items WHERE bulan = $1', [bulan + '-01']);
  return rows[0] ? rows[0].items : [];
}

async function saveK94Items(bulan, items) {
  const clean = (items || []).map(it => ({
    ds: (it && it.ds !== null && it.ds !== undefined) ? String(it.ds) : '',
    di: (it && it.di !== null && it.di !== undefined) ? String(it.di) : '',
    ket: (it && it.ket !== null && it.ket !== undefined) ? String(it.ket) : ''
  }));
  await pool.query(
    `INSERT INTO kpi_9_4_items (bulan, items, updated_at) VALUES ($1, $2::jsonb, now())
     ON CONFLICT (bulan) DO UPDATE SET items = EXCLUDED.items, updated_at = now()`,
    [bulan + '-01', JSON.stringify(clean)]
  );
}

async function getKpi9_4Data(access, bulanQuery) {
  const currentYm = currentYmWITA();
  let bulan = (typeof bulanQuery === 'string' && /^\d{4}-\d{2}$/.test(bulanQuery)) ? bulanQuery : null;
  // Default: bulan terakhir yang sudah diisi admin (kalau ada & dalam rentang).
  if (!bulan) {
    const latest = await latestDataYm([{ table: 'kpi_9_4_items', monthCol: 'bulan' }]);
    if (latest && ymToIndex(latest) >= ymToIndex(KUALITAS_START_YM)) bulan = latest;
  }
  if (!bulan) bulan = currentYm;
  const availableMonths = buildAvailableMonths(bulan > currentYm ? bulan : currentYm);
  ...
  const year = Number(yStr), monthIndex = Number(mStr) - 1;
  const monthTitle = `${MONTHS_TITLE[monthIndex]} ${year}`;
  const items = await loadK94Items(bulan);
  const meta = Object.assign({}, K94_DEFAULT_META, (await loadGlobalK94Meta()) || {});
  return { locked: false, availableMonths, bulan, year, monthIndex, monthTitle, items, meta };
}

// --- Unduh Excel 9.4 -- exceljs di server, alasan sama KPI lain. Meniru PERSIS
// file contoh "9.4 Laporan Ketidaksesuaian debit.xlsx" (sheet "Ketidaksesuaian
// debit 04"): kop "Lampiran : 9.4 LAPORAN KETIDAK SESUAIAN DEBIT", Bagian/Lokasi/
// Bulan + judul, header NO | DEBIT SEBELUMNYA | DEBIT HARI INI | KETERANGAN,
// tiap item = blok 3 baris (NO + debit sebelumnya/hari ini di baris pertama,
// KETERANGAN di-merge 3 baris), penandatangan 2 kolom (Menyetujui/Disetujui di
// kiri, Diketahui di kanan) & kode dokumen. Border data: HAIR horizontal,
// THIN vertikal (baris pertama atas THIN, item terakhir bawah THIN).
const K94_FONT = 'Calibri';
const K94_HAIR = { style: 'hair', color: { argb: 'FF000000' } };
const K94_COL_WIDTHS = [4.44, 18.33, 10.55, 10.00, 11.55, 8.33, 18.11, 17.11, 50.00];

function buildKpi9_4ExcelWorkbook(data) {
  const wb = new ExcelJS.Workbook();
  wb.calcProperties.fullCalcOnLoad = true;
  const ws = wb.addWorksheet('Ketidaksesuaian debit 04', {
    pageSetup: { orientation: 'landscape', paperSize: 5, fitToPage: false, scale: 90, fitToWidth: 1, fitToHeight: 1, blackAndWhite: false, draft: false }
  });
  ws.pageSetup.margins = { left: 0.7, right: 0.7, top: 0.75, bottom: 0.75, header: 0, footer: 0 };
  K94_COL_WIDTHS.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
  ...
  const bdT = (c, r, sides) => bdAdd(c, r, sides, THIN);
  const bdH = (c, r, sides) => bdAdd(c, r, sides, K94_HAIR);
  const setv = (c, r, v, font) => {
    const cell = CELL(c, r);
    if (v !== undefined && v !== null) cell.value = v;
    cell.font = font || { name: K94_FONT, size: 11 };
    return cell;
  };
  const ctr = (cell) => { cell.alignment = { horizontal: 'center', vertical: 'middle' }; return cell; };
  const cal12 = { name: 'Calibri', size: 12 };
  const cal12b = { name: 'Calibri', size: 12, bold: true };
  const cal11 = { name: 'Calibri', size: 11 };
  const cal11b = { name: 'Calibri', size: 11, bold: true };

  // --- Kop lampiran (baris 2-3) ---
  setv(2, 2, 'Lampiran     : 9.4 LAPORAN KETIDAK SESUAIAN DEBIT', cal12);
  CELL(2, 2).alignment = { horizontal: 'left' };
  MRG(2, 2, 9, 3);
  bdT(2, 2, 'tl');
  for (let i = 3; i <= 8; i++) bdT(i, 2, 't');
  bdT(9, 2, 'tr');
  bdT(2, 3, 'lb');
  for (let i = 3; i <= 8; i++) bdT(i, 3, 'b');
  bdT(9, 3, 'br');

  // --- Bagian/Lokasi/Bulan + judul (baris 4-6) ---
  setv(2, 4, 'Bagian', cal12);
  setv(3, 4, ': Sumber Air Baku', cal12);
  bdT(2, 4, 'l');
  MRG(6, 4, 9, 5);
  const title = setv(6, 4, 'LAPORAN KETIDAK SESUAIAN DEBIT', cal12);
  title.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  bdT(6, 4, 'tl'); bdT(7, 4, 't'); bdT(8, 4, 't'); bdT(9, 4, 'tr');
  bdT(6, 5, 'lb'); bdT(7, 5, 'b'); bdT(8, 5, 'b'); bdT(9, 5, 'br');

  setv(2, 5, 'Lokasi', cal12);
  setv(3, 5, ': Pump House Dan Sumur-Sumur', cal12);
  bdT(2, 5, 'l');

  setv(2, 6, 'Bulan', cal12);
  setv(3, 6, ': ' + data.monthTitle, cal12b);
  bdT(2, 6, 'lb'); bdT(3, 6, 'b'); bdT(4, 6, 'b'); bdT(5, 6, 'b');
  MRG(6, 6, 9, 6);
  bdT(6, 6, 'tlb'); bdT(7, 6, 'tb'); bdT(8, 6, 'tb'); bdT(9, 6, 'tbr');

  // --- Header tabel (baris 7-8) ---
  MRG(2, 7, 2, 8); ctr(setv(2, 7, 'NO', cal12));
  bdT(2, 7, 'tlr'); bdT(2, 8, 'lbr');
  MRG(3, 7, 4, 7); ctr(setv(3, 7, 'DEBIT', cal12));
  bdT(3, 7, 'tl'); bdT(4, 7, 'tr');
  MRG(3, 8, 4, 8); ctr(setv(3, 8, 'SEBELUMNYA', cal12));
  bdT(3, 8, 'lb'); bdT(4, 8, 'br');
  MRG(5, 7, 6, 7); ctr(setv(5, 7, 'DEBIT', cal12));
  bdT(5, 7, 'tl'); bdT(6, 7, 'tr');
  MRG(5, 8, 6, 8); ctr(setv(5, 8, 'HARI INI', cal12));
  bdT(5, 8, 'lb'); bdT(6, 8, 'br');
  MRG(7, 7, 9, 8); ctr(setv(7, 7, 'KETERANGAN', cal12));
  bdT(7, 7, 'tl'); bdT(8, 7, 't'); bdT(9, 7, 'tr');
  bdT(7, 8, 'lb'); bdT(8, 8, 'b'); bdT(9, 8, 'br');

  // --- Baris data: tiap item = blok 3 baris (pola item 1 di contoh) ---
  const items = data.items || [];
  const count = items.length;
  const writeItem = (r0, no, item, isLast) => {
    const ds = item ? String(item.ds || '') : '';
    const di = item ? String(item.di || '') : '';
    const ket = item ? String(item.ket || '') : '';
    const bBot = isLast ? THIN : K94_HAIR;

    // NO (B)
    ctr(setv(2, r0, no, cal12));
    MRG(2, r0 + 1, 2, r0 + 2);
    bdH(2, r0, 't'); bdT(2, r0, 'lr');
    bdT(2, r0 + 1, 'lr');
    bdT(2, r0 + 2, 'lr'); bdAdd(2, r0 + 2, 'b', bBot);

    // DEBIT SEBELUMNYA (C:D)
    MRG(3, r0, 4, r0); ctr(setv(3, r0, ds, cal12));
    MRG(3, r0 + 1, 4, r0 + 2);
    bdH(3, r0, 't'); bdT(3, r0, 'l'); bdH(4, r0, 't'); bdT(4, r0, 'r');
    bdT(3, r0 + 1, 'l'); bdT(4, r0 + 1, 'r');
    bdT(3, r0 + 2, 'l'); bdAdd(3, r0 + 2, 'b', bBot); bdT(4, r0 + 2, 'r'); bdAdd(4, r0 + 2, 'b', bBot);

    // DEBIT HARI INI (E:F)
    MRG(5, r0, 6, r0); ctr(setv(5, r0, di, cal12));
    MRG(5, r0 + 1, 6, r0 + 2);
    bdH(5, r0, 't'); bdT(5, r0, 'l'); bdH(6, r0, 't'); bdT(6, r0, 'r');
    bdT(5, r0 + 1, 'l'); bdT(6, r0 + 1, 'r');
    bdT(5, r0 + 2, 'l'); bdAdd(5, r0 + 2, 'b', bBot); bdT(6, r0 + 2, 'r'); bdAdd(6, r0 + 2, 'b', bBot);

    // KETERANGAN (G:I, merge 3 baris)
    MRG(7, r0, 9, r0 + 2);
    const kcell = setv(7, r0, ket, cal12);
    kcell.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
    bdH(7, r0, 't'); bdT(7, r0, 'l'); bdH(8, r0, 't'); bdH(9, r0, 't'); bdT(9, r0, 'r');
    bdT(7, r0 + 1, 'l'); bdT(9, r0 + 1, 'r');
    bdT(7, r0 + 2, 'l'); bdAdd(7, r0 + 2, 'b', bBot); bdAdd(8, r0 + 2, 'b', bBot); bdT(9, r0 + 2, 'r'); bdAdd(9, r0 + 2, 'b', bBot);
  };

  let r = 9;
  items.forEach((item, i) => { writeItem(r, i + 1, item, i === count - 1); r += 3; });
  const lastDataRow = count > 0 ? r - 1 : 9;
  if (count === 0) {
    // tidak ada item: tutup tabel dengan satu baris kosong ber-garis bawah
    for (let i = 2; i <= 9; i++) bdAdd(i, 9, 'b', THIN);
  }

  // --- Penandatangan 2 kolom & kode dokumen ---
  const meta = data.meta || {};
  const labelR = lastDataRow + 3;
  MRG(2, labelR, 3, labelR); ctr(setv(2, labelR, 'Menyetujui/Disetujui', cal12));
  ctr(setv(9, labelR, 'Diketahui', cal12));
  const roleR = labelR + 1;
  MRG(2, roleR, 3, roleR);
  const roleL = setv(2, roleR, meta.roleMenyetujui || '', cal11);
  roleL.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  const roleRc = setv(9, roleR, meta.roleDiketahui || '', cal11);
  roleRc.alignment = { horizontal: 'center', vertical: 'middle' };
  const nameR = roleR + 5;
  MRG(2, nameR, 3, nameR);
  const nameL = setv(2, nameR, meta.nameMenyetujui || '', cal11b);
  nameL.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  const nameRc = setv(9, nameR, meta.nameDiketahui || '', cal12b);
  nameRc.alignment = { horizontal: 'center', vertical: 'middle' };
  const footR = nameR + 3;
  MRG(2, footR, 3, footR);
  ctr(setv(2, footR, meta.footerCode || '', cal12b));
  bdT(2, footR, 'tlb'); bdT(3, footR, 'tbr');

  // Tinggi baris seragam persis contoh (15.75).
  for (let rr = 1; rr <= footR; rr++) ws.getRow(rr).height = 15.75;

  return wb.xlsx.writeBuffer();
}

// 9.7 Laporan Kondisi Air Sumur (apps/kpi-sab/kondisi-sumur-9-7.html).
//
// Satu workbook = 5 sheet (5 IPA, Hal 1-5), urutan & nama sheet persis contoh
// "9.7 Laporan Kondisi Air Sumur.xlsx": LvDbt Gunung Sari, LvDbt Prapatan,
// LvDbt Kp Damai, LvDbt Zamp, LvDbt KBU.
//
// DATA per IPA:
//   LEVEL statis/dinamis (m)  -> bulan lalu & bulan ini dari data web
//     sumur_level_readings (fetchSumurLevelRows).
//   REALITA = debit sumur     -> bulan lalu & bulan ini dari data web
//     sumur_debit_readings (fetchSumurDebitRows).
//   MERK / TYPE / NAME PLATE / KETERANGAN -> MANUAL admin (default dari
//     contoh supaya tidak perlu diketik ulang), disimpan per bulan di
//     kpi_9_7_items (rows JSONB) + catatan (teks bebas).
//   Tanggal tiap IPA         -> otomatis: Gunung Sari & Prapatan = Senin
//     minggu ke-1, Kp Damai/Zamp/Kampung Baru = Senin minggu ke-2 (kpi192Tanggal).
//   Penandatangan 3 kolom + kode dokumen "PTMBPP-IR-PRD.SAB/01-07" +
//     total halaman & nomor Hal per IPA -> kpi_9_7_meta (global). Tanda tangan
//     hanya dicetak di sheet terakhir (Hal 5 = LvDbt KBU).
// Unduhan Excel = persis contoh (tiap sheet punya config layout sendiri karena
// contohnya dibuat manual dengan posisi baris/merge yang berbeda-beda).
// ===========================================================================

const K97_META_KEY = 'global';
const K97_DEFAULT_META = {
  roleDiketahui: 'Supervisor Sumber Air Baku & Lingkungan',
  nameDiketahui: 'DARTO',
  roleMengetahui: 'Manajer Produksi',
  nameMengetahui: 'DEDY HERMAWAN S.M',
  namePelaksana: 'M RAHMAD HIDAYAT',
  footerCode: 'PTMBPP-IR-PRD.SAB/01-07',
  totalHalaman: '5',
  halaman: { gunung_sari: '1', prapatan: '2', kampung_damai: '3', zamp: '4', kampung_baru_ulu: '5' }
};

// 5 IPA urutan sheet contoh. minggu = nomor minggu di kpi192Tanggal untuk
// menghitung Tanggal laporan (hari = 1 = Senin).
const K97_IPAS = [
  { installation: 'gunung_sari', lokasi: 'IPA Gunung Sari', sheetName: 'LvDbt Gunung Sari', minggu: 1, hal: '1' },
  { installation: 'prapatan', lokasi: 'IPA PRAPATAN', sheetName: 'LvDbt Prapatan', minggu: 1, hal: '2' },
  { installation: 'kampung_damai', lokasi: 'IPA Kampung Damai', sheetName: 'LvDbt Kp Damai', minggu: 2, hal: '3' },
  { installation: 'zamp', lokasi: 'IPA ZAMP', sheetName: 'LvDbt Zamp', minggu: 2, hal: '4' },
  { installation: 'kampung_baru_ulu', lokasi: 'IPA Kampung Baru Ulu', sheetName: 'LvDbt KBU', minggu: 2, hal: '5' }
];

// Default manual per sumur (persis isian MERK/TYPE/NAME PLATE/KETERANGAN di
// contoh) -- dipakai kalau admin belum menyimpan isian bulan itu. Kunci:
// {installation}:{no}.
const K97_DEFAULTS = {
  gunung_sari: {
    1: { merk: 'GRUNDFOS', type: 'SP 17-11', namePlate: '17', keterangan: 'Stand By' },
    2: { merk: 'GRUNDFOS', type: 'SP 77-5', namePlate: '77', keterangan: '' },
    3: { merk: 'GRUNDFOS', type: 'SP 95-7', namePlate: '95', keterangan: '' },
    4: { merk: 'GRUNDFOS', type: 'SP 95-5', namePlate: '95', keterangan: '' },
    5: { merk: 'CRI', type: '22 KW', namePlate: '80', keterangan: 'Debit diatur agar sumur tidak keruh' },
    6: { merk: 'GRUNDFOS', type: 'SP 95-6', namePlate: '95', keterangan: '' },
    7: { merk: 'GRUNDFOS', type: 'SP 95-5', namePlate: '95', keterangan: '' }
  },
  prapatan: {
    1: { merk: 'Grundfost ', type: 'SP 95-7', namePlate: '95', keterangan: 'Debit Sumur diatur sesuai dengan kebutuhan IPA' },
    2: { merk: 'Grundfost ', type: 'SP 95-5', namePlate: '95', keterangan: 'Debit Sumur diatur sesuai dengan kebutuhan IPA' },
    3: { merk: 'Grundfost ', type: 'SP 95 -5', namePlate: '60', keterangan: 'Debit Sumur diatur sesuai dengan kebutuhan IPA' }
  },
  kampung_damai: {
    1: { merk: 'CRI', type: '18 kw', namePlate: '46', keterangan: '' },
    2: { merk: 'GRUNDFOS', type: 'SP 46-11', namePlate: '46', keterangan: '' },
    3: { merk: 'GRUNDFOS', type: 'SP 46-8', namePlate: '46', keterangan: '' },
    5: { merk: 'GRUNDFOS', type: 'SP 77-7', namePlate: '77', keterangan: '' }
  },
  zamp: {
    2: { merk: 'GRUNDFOST', type: 'SP 30-11', namePlate: '30', keterangan: '' },
    3: { merk: 'GRUNDFOST', type: 'SP 30-8', namePlate: '30', keterangan: '' }
  },
  kampung_baru_ulu: {
    1: { merk: 'GRUNDFOST', type: 'SP 46-13', namePlate: '46', keterangan: 'Sumur Tidak Diaktifkan' },
    2: { merk: 'GRUNDFOST', type: 'SP 30-10', namePlate: '30', keterangan: '' },
    3: { merk: 'CRI', type: '18 kW', namePlate: '64', keterangan: 'Debit Diatur Sesuai Kebutuhan IPA' }
  }
};

async function loadGlobalK97Meta() {
  const { rows } = await pool.query('SELECT * FROM kpi_9_7_meta WHERE period_key = $1', [K97_META_KEY]);
  if (!rows[0]) return null;
  const r = rows[0];
  return {
    roleDiketahui: r.role_diketahui || '',
    ...
    namePelaksana: r.name_pelaksana || '',
    footerCode: r.footer_code || '',
    totalHalaman: r.total_halaman || '',
    halaman: r.halaman || {}
  };
}

async function saveK97Meta(m) {
  const halaman = m.halaman && typeof m.halaman === 'object' ? m.halaman : {};
  await pool.query(
    `INSERT INTO kpi_9_7_meta (period_key, role_diketahui, name_diketahui, role_mengetahui, name_mengetahui, name_pelaksana, footer_code, total_halaman, halaman, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, now())
     ON CONFLICT (period_key) DO UPDATE SET
       role_diketahui = EXCLUDED.role_diketahui, name_diketahui = EXCLUDED.name_diketahui,
       role_mengetahui = EXCLUDED.role_mengetahui, name_mengetahui = EXCLUDED.name_mengetahui,
       name_pelaksana = EXCLUDED.name_pelaksana, footer_code = EXCLUDED.footer_code,
       total_halaman = EXCLUDED.total_halaman, halaman = EXCLUDED.halaman,
       updated_at = now()`,
    [K97_META_KEY, m.roleDiketahui || '', m.nameDiketahui || '', m.roleMengetahui || '', m.nameMengetahui || '', m.namePelaksana || '', m.footerCode || '', m.totalHalaman || '', JSON.stringify(halaman)]
  );
}

async function loadK97Items(bulan, installation) {
  const { rows } = await pool.query(
    'SELECT rows, catatan FROM kpi_9_7_items WHERE bulan = $1 AND installation = $2',
    [bulan + '-01', installation]
  );
  return rows[0] ? { rows: rows[0].rows, catatan: rows[0].catatan || '' } : null;
}

async function saveK97Items(bulan, installation, rows, catatan) {
  const clean = (rows || []).map(r => ({
    no: (r && r.no !== null && r.no !== undefined) ? String(r.no) : '',
    merk: (r && r.merk !== null && r.merk !== undefined) ? String(r.merk) : '',
    type: (r && r.type !== null && r.type !== undefined) ? String(r.type) : '',
    namePlate: (r && r.namePlate !== null && r.namePlate !== undefined) ? String(r.namePlate) : '',
    keterangan: (r && r.keterangan !== null && r.keterangan !== undefined) ? String(r.keterangan) : ''
  }));
  await pool.query(
    `INSERT INTO kpi_9_7_items (bulan, installation, rows, catatan, updated_at) VALUES ($1, $2, $3::jsonb, $4, now())
     ON CONFLICT (bulan, installation) DO UPDATE SET rows = EXCLUDED.rows, catatan = EXCLUDED.catatan, updated_at = now()`,
    [bulan + '-01', installation, JSON.stringify(clean), (catatan === null || catatan === undefined) ? '' : String(catatan)]
  );
}

// Nomor sumur dari nama well (mis. "Sumur_05_Penggalang" -> 5,
// "SUMUR 6 ( TLGS 2 )" -> 6). Fallback ke urutan index supaya tidak error.
function k97WellNo(name, fallbackIndex) {
  const m = String(name).match(/^Sumur_0*(\d+)/i) || String(name).match(/SUMUR\s*0*(\d+)/i);
  if (m) return Number(m[1]);
  return fallbackIndex;
}

function k97IndexByBulan(rows) {
  const map = {};
  (rows || []).forEach(r => { map[r.Bulan] = r; });
  return map;
}

// Bangun baris satu IPA: gabungkan debit + level (bulan & bulan-1) dengan
// isian manual. stored = null -> daftar sumur otomatis + default contoh;
// stored = { rows, catatan } -> daftar baris persis yang disimpan admin.
function k97ComposeRows(ipa, bulan, debit, level, stored) {
  const prevYm = indexToYm(ymToIndex(bulan) - 1);
  const debitCur = debit.rowsByBulan[bulan] || {};
  const debitPrev = debit.rowsByBulan[prevYm] || {};
  const levelCur = level.rowsByBulan[bulan] || {};
  const levelPrev = level.rowsByBulan[prevYm] || {};

  const debitWellByNo = {};
  debit.wells.forEach((w, i) => { debitWellByNo[k97WellNo(w, i + 1)] = w; });
  const levelWellByNo = {};
  level.wells.forEach((w, i) => { levelWellByNo[k97WellNo(w, i + 1)] = w; });

  const defs = K97_DEFAULTS[ipa.installation] || {};
  const storedRows = Array.isArray(stored && stored.rows) ? stored.rows : null;
  const val = (o, k) => (o[k] === null || o[k] === undefined ? null : o[k]);

  const buildRow = (no, s) => {
    const numNo = Number(no);
    const d = defs[String(no)] || {};
    const lw = levelWellByNo[numNo];
    const w = debitWellByNo[numNo];
    return {
      no: String(no),
      merk: s ? s.merk : (d.merk || ''),
      type: s ? s.type : (d.type || ''),
      namePlate: s ? s.namePlate : (d.namePlate != null ? d.namePlate : ''),
      keterangan: s ? s.keterangan : (d.keterangan || ''),
      levelLaluStatis: lw ? val(levelPrev, lw + '_Statis') : null,
      levelLaluDinamis: lw ? val(levelPrev, lw + '_Dinamis') : null,
      levelIniStatis: lw ? val(levelCur, lw + '_Statis') : null,
      levelIniDinamis: lw ? val(levelCur, lw + '_Dinamis') : null,
      realLalu: w ? val(debitPrev, w) : null,
      realIni: w ? val(debitCur, w) : null
    };
  };

  let rows;
  if (storedRows) {
    rows = storedRows.map(s => buildRow(s.no, s));
  } else {
    rows = debit.wells.map((w, i) => buildRow(k97WellNo(w, i + 1), null));
  }
  rows.sort((a, b) => Number(a.no) - Number(b.no));
  return { rows, catatan: (stored && stored.catatan) || '' };
}

function k97ComposeIpa(ipa, bulan, debit, level, stored, year, monthIndex) {
  const tanggal = formatTanggalId(kpi192Tanggal(year, monthIndex, 1, ipa.minggu));
  const built = k97ComposeRows(ipa, bulan, debit, level, stored);
  return {
    installation: ipa.installation,
    lokasi: ipa.lokasi,
    sheetName: ipa.sheetName,
    halDefault: ipa.hal,
    tanggal,
    rows: built.rows,
...
async function getKpi9_7Data(access, bulanQuery) {
  const currentYm = currentYmWITA();
  let bulan = (typeof bulanQuery === 'string' && /^\d{4}-\d{2}$/.test(bulanQuery)) ? bulanQuery : null;
  // Default: bulan terakhir yang ADA datanya (level & debit sumur).
  if (!bulan) {
    const latest = await latestDataYm([
      { table: 'sumur_level_readings', monthCol: 'bulan' },
      { table: 'sumur_debit_readings', monthCol: 'bulan' }
    ]);
    if (latest && ymToIndex(latest) >= ymToIndex(KUALITAS_START_YM)) bulan = latest;
  }
  if (!bulan || ymToIndex(bulan) < ymToIndex(KUALITAS_START_YM)) {
  ...
  const year = Number(yStr), monthIndex = Number(mStr) - 1;
  const monthTitle = `${MONTHS_TITLE[monthIndex]} ${year}`;
  const meta = Object.assign({}, K97_DEFAULT_META, (await loadGlobalK97Meta()) || {});

  const storedByInst = {};
  await Promise.all(K97_IPAS.map(async ipa => {
    storedByInst[ipa.installation] = await loadK97Items(bulan, ipa.installation);
  }));

  let ipas;
  if (!access.granted) {
    // Data contoh (terkunci): LEVEL & REALITA dummy, isian manual kosong/default.
    ipas = [];
    for (const ipa of K97_IPAS) {
      const debitWells = await fetchSumurWells(ipa.installation, 'debit');
      const levelWells = await fetchSumurWells(ipa.installation, 'level');
      const debit = { wells: debitWells, rowsByBulan: k97IndexByBulan(buildDummySumurDebitRows(debitWells, 80)) };
      const level = { wells: levelWells, rowsByBulan: k97IndexByBulan(buildDummySumurLevelRows(levelWells, 60)) };
      ipas.push(k97ComposeIpa(ipa, bulan, debit, level, null, year, monthIndex));
    }
  } else {
    const [debitRes, levelRes] = await Promise.all([
      Promise.all(K97_IPAS.map(ipa => fetchSumurDebitRows({ installation: ipa.installation }))),
      Promise.all(K97_IPAS.map(ipa => fetchSumurLevelRows({ installation: ipa.installation })))
    ]);
    ipas = K97_IPAS.map((ipa, i) => {
      const debit = { wells: debitRes[i].wells, rowsByBulan: k97IndexByBulan(debitRes[i].rows) };
      const level = { wells: levelRes[i].wells, rowsByBulan: k97IndexByBulan(levelRes[i].rows) };
      return k97ComposeIpa(ipa, bulan, debit, level, storedByInst[ipa.installation], year, monthIndex);
    });
    if (access && access.granted) await logViewerAction(access, 'kpi_9_7', 'view');
  }

  return { locked: !access.granted, availableMonths, bulan, year, monthIndex, monthTitle, ipas, meta };
}

// --- Unduh Excel 9.7 -- exceljs di server, alasan sama KPI lain. Meniru
// file contoh "9.7 Laporan Kondisi Air Sumur.xlsx": 5 sheet dengan nama &
// urutan persis contoh. Tiap sheet punya config layout sendiri (kop perusahaan
// ada/tidak, posisi baris, lebar kolom, range footer) karena contohnya dibuat
// manual dengan posisi yang tidak seragam antar sheet.
const K97_DOUBLE = { style: 'double', color: { argb: 'FF000000' } };
const K97_FONT = 'Calibri';
const K97_HAIR = { style: 'hair', color: { argb: 'FF000000' } };

const K97_SHEETS = [
  {
    sheet: 'LvDbt Gunung Sari', installation: 'gunung_sari', halDefault: '1',
    hasCompany: false, companyText: '', companyRow: 0,
    lampiranTop: 1, lampiranBottom: 2, bagRow: 3, lokRow: 4, tglRow: 5,
    titleEnd: 12, headTop: 6, dataTop: 10, baseRows: 7, catatanStart: 18, footerRow: 24,
    qText: 'Q ( m3 )', qWide: true, unit: '( m3/h )',
    firstTop: 'thin', colCount: 12, scale: 85, hasSignature: false,
    kodeRange: { c1: 2, c2: 4 }, halamanRange: { c1: 11, c2: 12 },
    widths: [11.33, 15.66, 10.66, 10.66, 10.44, 12.11, 11.11, 15.66, 10.66, 10.11, 10.66, 27.11]
  },
  {
    sheet: 'LvDbt Prapatan', installation: 'prapatan', halDefault: '2',
    hasCompany: true, companyText: 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG KOTA BALIKPAPAN', companyRow: 2,
    lampiranTop: 3, lampiranBottom: 4, bagRow: 5, lokRow: 6, tglRow: 7,
    titleEnd: 12, headTop: 8, dataTop: 12, baseRows: 3, catatanStart: 17, footerRow: 29,
    qText: 'Q', qWide: false, unit: '',
    firstTop: 'double', colCount: 14, scale: 90, hasSignature: false,
    kodeRange: { c1: 2, c2: 4 }, halamanRange: { c1: 11, c2: 13 },
    widths: [11.89, 17.55, 14.44, 11.11, 11.33, 11.66, 10.89, 14.33, 8.66, 8.66, 12.89, 12.66, 9.11, 12.55]
  },
  {
    sheet: 'LvDbt Kp Damai', installation: 'kampung_damai', halDefault: '3',
    hasCompany: false, companyText: '', companyRow: 0,
    lampiranTop: 2, lampiranBottom: 3, bagRow: 4, lokRow: 5, tglRow: 6,
    titleEnd: 13, headTop: 7, dataTop: 11, baseRows: 4, catatanStart: 17, footerRow: 29,
    qText: 'Q ( M3/H )', qWide: true, unit: '( m3/h )',
    firstTop: 'double', colCount: 14, scale: 90, hasSignature: false,
    kodeRange: { c1: 2, c2: 4 }, halamanRange: { c1: 13, c2: 14 }, halamanRowOffset: -1,
    widths: [10.11, 14.44, 10.66, 12.33, 11.66, 12.44, 13.89, 12.33, 10.66, 10.66, 10.66, 10.66, 20.66, 12.55]
  },
  {
    sheet: 'LvDbt Zamp', installation: 'zamp', halDefault: '4',
    hasCompany: true, companyText: 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG KOTA BALIKPAPAN', companyRow: 2,
    lampiranTop: 3, lampiranBottom: 4, bagRow: 5, lokRow: 6, tglRow: 7,
    titleEnd: 12, headTop: 8, dataTop: 12, baseRows: 2, catatanStart: 0, footerRow: 26,
    qText: 'Q (m3/h)', qWide: true, unit: '( m3/h )',
    firstTop: 'double', colCount: 14, scale: 90, hasSignature: false,
    kodeRange: { c1: 2, c2: 4 }, halamanRange: { c1: 11, c2: 12 }, halamanRowOffset: -1,
    widths: [11.89, 17.55, 18.33, 11.33, 11.11, 11.44, 10.89, 12.11, 8.66, 8.66, 12.89, 12.66, 9.11, 12.55]
  },
  {
    sheet: 'LvDbt KBU', installation: 'kampung_baru_ulu', halDefault: '5',
    hasCompany: true, companyText: 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG KOTA BALIKPAPAN', companyRow: 2,
    lampiranTop: 3, lampiranBottom: 4, bagRow: 5, lokRow: 6, tglRow: 7,
    titleEnd: 12, headTop: 8, dataTop: 12, baseRows: 3, catatanStart: 17, footerRow: 28,
    titleText: '9.7 LAPORAN KONDISI AIR SUMUR',
    qText: 'Q (m3)', qWide: true, unit: '( m3/h )',
    firstTop: 'double', colCount: 14, scale: 90, hasSignature: true,
    kodeRange: { c1: 1, c2: 3 }, halamanRange: { c1: 12, c2: 14 },
    widths: [18.33, 13.33, 14, 14.33, 12.11, 15.89, 8.66, 8.66, 12.89, 12.66, 8.66, 12.55, 8.66, 8.66]
  }
];

function buildKpi9_7ExcelWorkbook(data) {
  const wb = new ExcelJS.Workbook();
  wb.calcProperties.fullCalcOnLoad = true;
  data.ipas.forEach((ipa, idx) => {
    const cfg = K97_SHEETS[idx] || K97_SHEETS[0];
    buildK97Sheet(wb, cfg, ipa, data);
  });
  return wb.xlsx.writeBuffer();
}

function buildK97Sheet(wb, cfg, ipa, data) {
  const ws = wb.addWorksheet(cfg.sheet, {
    pageSetup: { orientation: 'landscape', paperSize: 5, fitToPage: false, scale: cfg.scale, fitToWidth: 1, fitToHeight: 1, blackAndWhite: false, draft: false }
  });
  ws.pageSetup.margins = { left: 0.4, right: 0.4, top: 0.5, bottom: 0.5, header: 0, footer: 0 };
  cfg.widths.forEach((w, i) => { ws.getColumn(i + 1).width = w; });

  const meta = data.meta || {};
  const totalHalaman = meta.totalHalaman || '5';
  const halNo = (meta.halaman || {})[ipa.installation] || cfg.halDefault;
  const rows = ipa.rows;
  const colCount = cfg.colCount;
  const dataTop = cfg.dataTop;
  const shift = Math.max(0, rows.length - cfg.baseRows);
  ...
  const setv = (c, r, v, font, numFmt) => {
    const cell = CELL(c, r);
    if (v !== undefined && v !== null) cell.value = v;
    cell.font = font || { name: K97_FONT, size: 11 };
    if (numFmt) cell.numFmt = numFmt;
    return cell;
  };
  const ctr = (cell) => { cell.alignment = { horizontal: 'center', vertical: 'middle' }; return cell; };
  ...
  const ar10 = { name: 'Arial', size: 10 };
  const ar10b = { name: 'Arial', size: 10, bold: true };
  // Bulatkan maksimal 2 angka desimal (permintaan user) -- dipakai utk nilai
  // LEVEL & REALITA serta totalnya, supaya sel & SUM di Excel tidak panjang.
  const r2 = (v) => (v === null || v === undefined || isNaN(v)) ? v : Math.round(v * 100) / 100;

  // --- Kop ---
  if (cfg.hasCompany) {
    setv(1, cfg.companyRow, cfg.companyText, cal11);
    for (let c = 1; c <= colCount; c++) bdT(c, cfg.companyRow, 'tb');
    bdT(1, cfg.companyRow, 'l'); bdT(colCount, cfg.companyRow, 'r');
  }
  // Lampiran (merge semua kolom, baris lampiranTop..lampiranBottom)
  MRG(1, cfg.lampiranTop, colCount, cfg.lampiranBottom);
  ctr(setv(1, cfg.lampiranTop, 'LAMPIRAN : 9.7 Laporan Kondisi Air Sumur', cal11));
  for (let c = 1; c <= colCount; c++) bdT(c, cfg.lampiranTop, 't');
  for (let c = 1; c <= colCount; c++) bdT(c, cfg.lampiranBottom, 'b');
  bdT(1, cfg.lampiranTop, 'l'); bdT(colCount, cfg.lampiranTop, 'r');
  bdT(1, cfg.lampiranBottom, 'l'); bdT(colCount, cfg.lampiranBottom, 'r');

  // Bagian / Lokasi / Tanggal + judul + Hal
  const b = cfg.bagRow, l = cfg.lokRow, t = cfg.tglRow;
  setv(1, b, 'Bagian', cal11); setv(2, b, ': Sumber Air Baku', cal11);
  setv(1, l, 'Lokasi', cal11); setv(2, l, ': ' + ipa.lokasi, cal11);
  setv(1, t, 'Tanggal', cal11); setv(2, t, ': ' + ipa.tanggal, cal11b);
  MRG(7, b, cfg.titleEnd, l);
  ctrw(setv(7, b, cfg.titleText || 'LAPORAN KONDISI AIR SUMUR', cal11));
  MRG(7, t, cfg.titleEnd, t);
  ctr(setv(7, t, 'Hal : ' + halNo + '  Dari :  ' + totalHalaman, cal11));
  for (let c = 1; c <= colCount; c++) bdT(c, t, 'b');
  for (let r = b; r <= t; r++) { bdT(1, r, 'l'); bdT(colCount, r, 'r'); }

  // --- Header tabel (4 baris) ---
  const h = cfg.headTop;
  MRG(1, h, 1, h + 3); ctrw(setv(1, h, 'No', cal11));
  MRG(2, h, 2, h + 3); ctrw(setv(2, h, 'MERK', cal11));
  MRG(3, h, 3, h + 3); ctrw(setv(3, h, 'TYPE', cal11));
  MRG(4, h, 6, h); ctrw(setv(4, h, 'LEVEL (m)', cal11));
  if (cfg.qWide) {
    MRG(8, h, 10, h); ctrw(setv(8, h, cfg.qText, cal11));
  } else {
    ctrw(setv(8, h, cfg.qText, cal11));
    MRG(9, h, 10, h + 1); ctrw(setv(9, h, 'REALITA (m3/jam)', cal11));
  }
  MRG(11, h, colCount, h + 3); ctrw(setv(11, h, 'KETERANGAN', cal11));

  if (cfg.qWide) {
    MRG(4, h + 1, 5, h + 1); ctrw(setv(4, h + 1, 'BULAN LALU', cal11));
    MRG(6, h + 1, 7, h + 1); ctrw(setv(6, h + 1, 'BULAN INI', cal11));
    MRG(8, h + 1, 8, h + 2); ctrw(setv(8, h + 1, 'NAME PLATE', cal11));
    MRG(9, h + 1, 10, h + 1); ctrw(setv(9, h + 1, 'REALITA', cal11));
    ctrw(setv(9, h + 2, 'Bln.Lalu', cal11));
    ctrw(setv(10, h + 2, 'Bln.Ini', cal11));
    ctrw(setv(8, h + 3, cfg.unit, cal11));
    ctrw(setv(9, h + 3, cfg.unit, cal11));
    ctrw(setv(10, h + 3, cfg.unit, cal11));
  } else {
    // sheet Prapatan (slim): BULAN LALU/INI 2 baris, NAME PLATE 1 baris,
    // REALITA (m3/jam) merge 2x2, Bln.Lalu/Bln.Ini di baris ke-4.
    MRG(4, h + 1, 5, h + 2); ctrw(setv(4, h + 1, 'BULAN LALU', cal11));
    MRG(6, h + 1, 7, h + 2); ctrw(setv(6, h + 1, 'BULAN INI', cal11));
    ctrw(setv(8, h + 1, 'NAME PLATE', cal11));
    ctrw(setv(9, h + 3, 'Bln.Lalu', cal11));
    ctrw(setv(10, h + 3, 'Bln.Ini', cal11));
  }
  ctrw(setv(4, h + 3, 'STATIS', cal11));
  ctrw(setv(5, h + 3, 'DYNAMIS', cal11));
  ctrw(setv(6, h + 3, 'STATIS', cal11));
  ctrw(setv(7, h + 3, 'DYNAMIS', cal11));

  // grid header (thin)
  for (let r = h; r <= h + 3; r++) {
    for (let c = 1; c <= colCount; c++) {
      const bd = CELL(c, r).border || {};
      bd.top = THIN; bd.left = THIN; bd.right = THIN; bd.bottom = THIN;
      CELL(c, r).border = bd;
    }
  }
  ...
  rows.forEach((row, i) => {
    const r = dataTop + i;
    const topS = i === 0 ? (cfg.firstTop === 'double' ? K97_DOUBLE : THIN) : K97_HAIR;
    const botS = i === rows.length - 1 ? THIN : K97_HAIR;
    for (let c = 1; c <= colCount; c++) {
      const bd = CELL(c, r).border || {};
      bd.top = topS; bd.bottom = botS;
      if (c === 1) bd.left = THIN;
      bd.right = THIN;
      CELL(c, r).border = bd;
    }
    setv(1, r, row.no, cal11); ctr(CELL(1, r));
    setv(2, r, row.merk, cal11); ctr(CELL(2, r));
    setv(3, r, row.type, cal11); ctr(CELL(3, r));
    setv(4, r, row.levelLaluStatis == null ? '-' : r2(row.levelLaluStatis), cal11); ctr(CELL(4, r));
    setv(5, r, row.levelLaluDinamis == null ? '-' : r2(row.levelLaluDinamis), cal11); ctr(CELL(5, r));
    setv(6, r, row.levelIniStatis == null ? '-' : r2(row.levelIniStatis), cal11); ctr(CELL(6, r));
    setv(7, r, row.levelIniDinamis == null ? '-' : r2(row.levelIniDinamis), cal11); ctr(CELL(7, r));
    // NAME PLATE ditulis ANGKA (bukan teks) supaya baris Total debit (SUM)
    // ikut menghitungnya; kalau kosong dibiarkan blank.
    var np = row.namePlate;
    var npNum = Number(np);
    setv(8, r, (np !== '' && np != null && !isNaN(npNum)) ? npNum : (np == null || np === '' ? null : np), cal11); ctr(CELL(8, r));
    setv(9, r, row.realLalu == null ? '-' : r2(row.realLalu), cal11); ctr(CELL(9, r));
    setv(10, r, row.realIni == null ? '-' : r2(row.realIni), cal11); ctr(CELL(10, r));
    MRG(11, r, colCount, r);
    const kc = setv(11, r, row.keterangan, cal11);
    kc.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  });

  // --- Total debit ---
  const tr = dataTop + rows.length;
  MRG(6, tr, 7, tr); ctr(setv(6, tr, 'Total debit', ar10b));
  const last = dataTop + rows.length - 1;
  if (rows.length) {
    // Total dihitung dari nilai yang SUDAH dibulatkan 2 desimal (sama dengan
    // isi selnya), supaya hasil cached konsisten kalau Excel menghitung ulang.
    const sumR2 = (key) => rows.reduce((s, r) => {
      const v = Number(r[key]);
      return s + (Number.isFinite(v) ? Math.round(v * 100) / 100 : 0);
    }, 0);
    setv(8, tr, { formula: `SUM(H${dataTop}:H${last})`, result: r2(sumR2('namePlate')) }, ar10b); ctr(CELL(8, tr));
    setv(9, tr, { formula: `SUM(I${dataTop}:I${last})`, result: r2(sumR2('realLalu')) }, ar10b); ctr(CELL(9, tr));
    setv(10, tr, { formula: `SUM(J${dataTop}:J${last})`, result: r2(sumR2('realIni')) }, ar10b); ctr(CELL(10, tr));
  }
  for (let c = 1; c <= colCount; c++) {
    const bd = CELL(c, tr).border || {};
    bd.top = THIN; bd.left = THIN; bd.right = THIN; bd.bottom = K97_DOUBLE;
    CELL(c, tr).border = bd;
  }

  // --- Catatan ---
  const catRow = cfg.catatanStart ? (cfg.catatanStart + shift) : 0;
  if (catRow) {
    setv(2, catRow, 'Catatan :', cal11);
    const lines = String(ipa.catatan || '').split('\n').map(s => s.trim()).filter(Boolean);
    lines.forEach((ln, i) => {
      setv(2, catRow + 1 + i, '*', cal11);
      setv(3, catRow + 1 + i, ln, cal11);
    });
  }

  // --- Tanda tangan (hanya sheet terakhir / Hal 5) ---
  const fr = cfg.footerRow + shift;
  if (cfg.hasSignature) {
    const lr = fr - 8;
    MRG(1, lr, 2, lr); ctr(setv(1, lr, 'Diketahui,', cal11));
    MRG(1, lr + 1, 2, lr + 2); ctrw(setv(1, lr + 1, meta.roleDiketahui || '', ar10));
    MRG(1, lr + 5, 2, lr + 5); ctr(setv(1, lr + 5, meta.nameDiketahui || '', ar10b));
    MRG(6, lr + 1, 8, lr + 1); ctr(setv(6, lr + 1, 'Mengetahui/Menyetujui', cal11));
    MRG(6, lr + 2, 8, lr + 2); ctr(setv(6, lr + 2, meta.roleMengetahui || '', cal11));
    MRG(6, lr + 6, 8, lr + 6); ctr(setv(6, lr + 6, meta.nameMengetahui || '', ar10b));
    MRG(12, lr, 14, lr); ctr(setv(12, lr, 'Pelaksana', cal11));
    MRG(12, lr + 5, 14, lr + 5); ctr(setv(12, lr + 5, meta.namePelaksana || '', cal11b));
  }

  // --- Footer: kode dokumen + Halaman (di contoh, Hal 3 & 4 "Halaman X"-nya
  // satu baris DI ATAS kode dokumen, sheet lain sebaris). ---
  MRG(cfg.kodeRange.c1, fr, cfg.kodeRange.c2, fr);
  ctr(setv(cfg.kodeRange.c1, fr, meta.footerCode || '', cal11b));
  bdT(cfg.kodeRange.c1, fr, 'tlb');
  for (let c = cfg.kodeRange.c1 + 1; c <= cfg.kodeRange.c2; c++) bdT(c, fr, 'tb');
  bdT(cfg.kodeRange.c2, fr, 'trb');
  const halFr = fr + (cfg.halamanRowOffset || 0);
  MRG(cfg.halamanRange.c1, halFr, cfg.halamanRange.c2, halFr);
  ctr(setv(cfg.halamanRange.c1, halFr, 'Halaman ' + halNo, cal11));

  // Tinggi baris seragam persis contoh (15.75).
  for (let r = 1; r <= fr; r++) ws.getRow(r).height = 15.75;
}

// 9.5 Laporan Monitoring Pipa Transmisi (apps/kpi-sab/monitoring-9-5.html).
//
// SATU sheet "MONITORING PIPA 05" per bulan. Dua baris monitoring TETAP:
//   NO 1 = Selasa minggu ke-2, NO 2 = Selasa minggu ke-4 (logika sama 18.6
//   Jalur Pipa Transmisi, pakai kpi192Tanggal). Tanggal otomatis dari bulan.
// KEADAAN (AIR VALVE / VALVE WASH OUT / PERPIPAAN) & KETERANGAN DIISI ADMIN
// manual per bulan (tidak ada data otomatis -- pola 9.4, semua viewer bisa
// lihat, admin edit + unduh).
//   items -> per bulan di kpi_9_5_items (rows JSONB): [{ no, airValve,
//     valveWashOut, perpipaan, keterangan }].
//   meta  -> global kpi_9_5_meta: penandatangan 3 blok (Mengetahui [kiri] /
//     Mengetahui-Menyetujui [tengah] / Pelaksana [kanan, daftar multi-baris]),
//     kode dokumen "PTMBPP-IR-PRD.SAB/01-05", Halaman default "1 dari 1".
// Unduhan Excel persis contoh "9.5 Laporan Monitoring Pipa Transmisi.xlsx".
// ===========================================================================

const K95_META_KEY = 'global';
const K95_DEFAULT_META = {
  roleMengetahui: 'Supervisor Sumber Air Baku & Lingkungan',
  nameMengetahui: 'DARTO',
  roleMenyetujui: 'Manajer Produksi',
  nameMenyetujui: 'DEDY HERMAWAN S.M',
  pelaksana: 'JAROT H\nM RAHMAD H\nRIAN NAZUAR E',
  footerCode: 'PTMBPP-IR-PRD.SAB/01-05',
  halaman: '1',
  totalHalaman: '1'
};

async function loadGlobalK95Meta() {
  const { rows } = await pool.query('SELECT * FROM kpi_9_5_meta WHERE period_key = $1', [K95_META_KEY]);
  if (!rows[0]) return null;
  const r = rows[0];
  return {
    roleMengetahui: r.role_mengetahui || '',
...
async function saveK95Meta(m) {
  await pool.query(
    `INSERT INTO kpi_9_5_meta (period_key, role_mengetahui, name_mengetahui, role_menyetujui, name_menyetujui, pelaksana, footer_code, total_halaman, halaman, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, now())
     ON CONFLICT (period_key) DO UPDATE SET
       role_mengetahui = EXCLUDED.role_mengetahui, name_mengetahui = EXCLUDED.name_mengetahui,
       role_menyetujui = EXCLUDED.role_menyetujui, name_menyetujui = EXCLUDED.name_menyetujui,
       pelaksana = EXCLUDED.pelaksana, footer_code = EXCLUDED.footer_code,
       total_halaman = EXCLUDED.total_halaman, halaman = EXCLUDED.halaman,
       updated_at = now()`,
    [K95_META_KEY, m.roleMengetahui || '', m.nameMengetahui || '', m.roleMenyetujui || '', m.nameMenyetujui || '', m.pelaksana || '', m.footerCode || '', m.totalHalaman || '', m.halaman || '']
  );
}

async function loadK95Items(bulan) {
  const { rows } = await pool.query('SELECT rows FROM kpi_9_5_items WHERE bulan = $1', [bulan + '-01']);
  return rows[0] ? rows[0].rows : [];
}

async function saveK95Items(bulan, items) {
  const clean = (items || []).map(r => ({
    no: (r && r.no !== null && r.no !== undefined) ? String(r.no) : '',
    airValve: (r && r.airValve !== null && r.airValve !== undefined) ? String(r.airValve) : '',
    valveWashOut: (r && r.valveWashOut !== null && r.valveWashOut !== undefined) ? String(r.valveWashOut) : '',
    perpipaan: (r && r.perpipaan !== null && r.perpipaan !== undefined) ? String(r.perpipaan) : '',
    keterangan: (r && r.keterangan !== null && r.keterangan !== undefined) ? String(r.keterangan) : ''
  }));
  await pool.query(
    `INSERT INTO kpi_9_5_items (bulan, rows, updated_at) VALUES ($1, $2::jsonb, now())
     ON CONFLICT (bulan) DO UPDATE SET rows = EXCLUDED.rows, updated_at = now()`,
    [bulan + '-01', JSON.stringify(clean)]
  );
}

async function getKpi9_5Data(access, bulanQuery) {
  const currentYm = currentYmWITA();
  let bulan = (typeof bulanQuery === 'string' && /^\d{4}-\d{2}$/.test(bulanQuery)) ? bulanQuery : null;
  // Default: bulan terakhir yang sudah diisi admin (kalau ada & dalam rentang).
  if (!bulan) {
    const latest = await latestDataYm([{ table: 'kpi_9_5_items', monthCol: 'bulan' }]);
    if (latest && ymToIndex(latest) >= ymToIndex(KUALITAS_START_YM)) bulan = latest;
  }
  if (!bulan) bulan = currentYm;
  const availableMonths = buildAvailableMonths(bulan > currentYm ? bulan : currentYm);
  ...
  // Tanggal otomatis: Selasa minggu ke-2 (NO 1) & Selasa minggu ke-4 (NO 2).
  const tgl1 = formatTanggalId(kpi192Tanggal(year, monthIndex, 2, 2));
  const tgl2 = formatTanggalId(kpi192Tanggal(year, monthIndex, 2, 4));
  const stored = await loadK95Items(bulan);
  const storedMap = {};
  stored.forEach(r => { storedMap[String(r.no)] = r; });

  const rows = ['1', '2'].map(no => {
    const s = storedMap[no] || {};
    return {
      no: no,
      tanggal: no === '1' ? tgl1 : tgl2,
      airValve: s.airValve || '',
      valveWashOut: s.valveWashOut || '',
      perpipaan: s.perpipaan || '',
      keterangan: s.keterangan || ''
    };
  });

  const meta = Object.assign({}, K95_DEFAULT_META, (await loadGlobalK95Meta()) || {});
  return { locked: false, availableMonths, bulan, year, monthIndex, monthTitle, rows, meta };
}

// --- Unduh Excel 9.5 -- exceljs di server, alasan sama KPI lain. Meniru PERSIS
// file contoh "9.5 Laporan Monitoring Pipa Transmisi.xlsx" (sheet "MONITORING
// PIPA 05"): kop "PERUSAHAAN UMUM...", Nomor/No.Revisi/Tgl.Revisi + judul
// "9.5 LAPORAN MONITORING PIPA TRANSMISI" + "Hal : 1 dari : 1", Bagian/Lokasi/
// Bulan, header NO | TANGGAL | KEADAAN (AIR VALVE | VALVE WASH OUT | PERPIPAAN)
// | KETERANGAN, dua blok monitoring (Selasa mgg ke-2 & ke-4; tinggi blok =
// jumlah baris keterangan), border data THIN vertikal (no garis horisontal),
// penandatangan 3 blok & kode dokumen di footer.
const K95_COL_WIDTHS = [8.66, 21.66, 18, 19, 20.33, 22.44, 23.11, 67.33];

function buildKpi9_5ExcelWorkbook(data) {
  const wb = new ExcelJS.Workbook();
  wb.calcProperties.fullCalcOnLoad = true;
  const ws = wb.addWorksheet('MONITORING PIPA 05', {
    pageSetup: { orientation: 'landscape', paperSize: 5, fitToPage: true, fitToWidth: 1, fitToHeight: 1, blackAndWhite: false, draft: false }
  });
  ws.pageSetup.margins = { left: 0.95, right: 0.7, top: 0.75, bottom: 0.75, header: 0, footer: 0 };
  K95_COL_WIDTHS.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
  ...
  const meta = data.meta || {};
  const hal = meta.halaman || '1';
  const totalHal = meta.totalHalaman || '1';
  const rows = data.rows || [];

  // Keterangan -> baris tampilan. Baris logis (dipisah "\n") dipecah kalau
  // lebih dari ~90 karakter (di batas kata) supaya tidak kepotong di dalam
  // sel merge F:H yang lebarnya ~113 unit; tiap potongan = 1 baris di Excel.
  const ketLines = (ket) => {
    const out = [];
    String(ket || '').split('\n').forEach(l => {
      let s = l;
      while (s.length > 90) {
        let cut = s.lastIndexOf(' ', 90);
        if (cut < 40) cut = 90;
  ...
  // --- Kop (baris 2-10) ---
  setv(1, 2, 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG KOTA BALIKPAPAN', ar10);
  for (let c = 1; c <= 8; c++) bdT(c, 2, 'tb');
  bdT(1, 2, 'l'); bdT(8, 2, 'r');

  // Baris 3-4 kosong.

  // Baris 5-7: Nomor / No.Revisi / Tgl.Revisi (kiri) + judul + Hal (kanan).
  setv(1, 5, 'Nomor', ar10); setv(2, 5, ':', ar10);
  setv(1, 6, 'No.Revisi', ar10); setv(2, 6, ':', ar10);
  setv(1, 7, 'Tgl.Revisi', ar10); setv(2, 7, ':', ar10);
  for (let c = 1; c <= 4; c++) bdT(c, 5, 't');
  for (let c = 1; c <= 4; c++) bdT(c, 7, 'b');
  for (let r = 5; r <= 7; r++) { bdT(1, r, 'l'); bdT(4, r, 'r'); }
  MRG(5, 5, 8, 6);
  ctrw(setv(5, 5, '9.5 LAPORAN MONITORING PIPA TRANSMISI', ar10));
  for (let c = 5; c <= 8; c++) bdT(c, 5, 't');
  for (let c = 5; c <= 8; c++) bdT(c, 6, 'b');
  bdT(5, 5, 'l'); bdT(8, 5, 'r'); bdT(5, 6, 'l'); bdT(8, 6, 'r');
  MRG(5, 7, 8, 7);
  ctr(setv(5, 7, 'Hal : ' + hal + '  dari :  ' + totalHal, ar10));
  for (let c = 5; c <= 8; c++) bdT(c, 7, 'tb');
  bdT(5, 7, 'l'); bdT(8, 7, 'r');

  // Baris 8-10: Bagian / Lokasi / Bulan.
  setv(1, 8, 'Bagian', ar10); setv(2, 8, ': Sumber Air Baku', ar10);
  setv(1, 9, 'Lokasi', ar10); setv(2, 9, ': Km 12 - Kampung Damai', ar10);
  setv(1, 10, 'Bulan', ar10); setv(2, 10, ': ' + data.monthTitle, ar10b);
  for (let c = 1; c <= 8; c++) bdT(c, 10, 'b');
  for (let r = 8; r <= 10; r++) { bdT(1, r, 'l'); bdT(8, r, 'r'); }

  // --- Header tabel (baris 11-12) ---
  MRG(1, 11, 1, 12); ctrw(setv(1, 11, 'NO', ar10));
  MRG(2, 11, 2, 12); ctrw(setv(2, 11, 'TANGGAL', ar10));
  MRG(3, 11, 5, 11); ctrw(setv(3, 11, 'KEADAAN', ar10));
  ctrw(setv(3, 12, 'AIR VALVE', ar10));
  ctrw(setv(4, 12, 'VALVE WASH OUT', ar10));
  ctrw(setv(5, 12, 'PERPIPAAN', ar10));
  MRG(6, 11, 8, 12); ctrw(setv(6, 11, 'KETERANGAN', ar10));
  // Border header mengikuti contoh: NO/TANGGAL & KETERANGAN kotak TANPA garis
  // bawah (garis bawah header digambar di baris 13); KEADAAN kotak penuh;
  // sub-header AIR VALVE/VALVE WASH OUT/PERPIPAAN kotak penuh.
  const setHb = (c, r, sides) => {
    const b = CELL(c, r).border || {};
    ['top', 'left', 'bottom', 'right'].forEach(s => { if (sides.indexOf(s[0]) >= 0) b[s] = THIN; });
    CELL(c, r).border = b;
  };
  setHb(1, 11, 'tlr'); setHb(2, 11, 'tlr'); setHb(3, 11, 'tlbr');
  setHb(3, 12, 'tlr'); setHb(4, 12, 'tlr'); setHb(5, 12, 'tlr');
  setHb(6, 11, 'tlr');
  // Baris 13: penutup header (garis atas + tepi kiri/kanan, pola contoh).
  for (let c = 1; c <= 5; c++) bdT(c, 13, 'tlr');
  bdT(6, 13, 'tl'); bdT(7, 13, 't'); bdT(8, 13, 'tr');

  // --- Blok data (2 monitoring) ---
  const dataStart = 14;
  let nextRow = dataStart;
  rows.forEach((r, idx) => {
    const lines = ketLines(r.keterangan);
    const height = Math.max(4, lines.length + 1);
    for (let i = 0; i < height; i++) {
      const rr = nextRow + i;
      for (let c = 1; c <= 5; c++) bdT(c, rr, 'lr');
      MRG(6, rr, 8, rr);
      const kcell = setv(6, rr, lines[i] != null ? lines[i] : null, tnr12);
      if (kcell) kcell.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
      // Border keterangan (merge F:H): tepi kiri & kanan saja -- DI-SET SETELAH
      // merge supaya border sel merge tidak rusak (ExcelJS menimpa border slave).
      bdT(6, rr, 'lr');
      if (i === 0) {
        ctr(setv(1, rr, r.no, ar10));
        ctr(setv(2, rr, r.tanggal, ar10));
        ctr(setv(3, rr, r.airValve, ar10));
        ctr(setv(4, rr, r.valveWashOut, ar10));
        ctr(setv(5, rr, r.perpipaan, ar10));
      }
    }
    nextRow += height;
    // Baris pemisah antar blok (blok terakhir sekaligus penutup tabel).
    for (let c = 1; c <= 5; c++) bdT(c, nextRow, 'lr');
    bdT(6, nextRow, 'l'); bdT(8, nextRow, 'r');
    nextRow++;
  });
  const closureRow = nextRow - 1;
  for (let c = 1; c <= 8; c++) bdT(c, closureRow, 'b');

  // --- Penandatangan 3 blok + kode dokumen ---
  // Nama pelaksana (disimpan polos, dipisah baris). Format tampil "1. NAMA •••".
  const pelLines = String(meta.pelaksana || '').split('\n')
    .map(l => l.replace(/^\d+\.\s*/, '').replace(/[.…\s]+$/, '').trim())
    .filter(l => l !== '');
  const sigLabel = closureRow + 3;
  const pelRows = pelLines.map((_, i) => sigLabel + (i === 0 ? 2 : (i === 1 ? 4 : 6 + (i - 1) * 2)));
  const lastPel = pelRows.length ? pelRows[pelRows.length - 1] : sigLabel + 2;
  // Nama & kode dokumen harus DI BAWAH blok peran (sigLabel+4..sigLabel+7) dan
  // baris pelaksana terakhir, supaya merge tidak bertabrakan saat daftar
  // pelaksana pendek (mis. cuma 2 baris).
  const nameRow = Math.max(lastPel, sigLabel + 7) + 2;
  const codeRow = nameRow + 2;

  // Pelaksana (kanan)
  MRG(7, sigLabel, 8, sigLabel);
  ctr(setv(7, sigLabel, 'Pelaksana', ar10));
  pelLines.forEach((line, i) => {
    const pr = pelRows[i];
    const txt = (i + 1) + '. ' + line + ' ' + '.'.repeat(Math.max(6, 45 - String(line).length));
    const cell = setv(7, pr, txt, ar10b);
    cell.alignment = { horizontal: 'left', vertical: 'middle' };
  });

  // Mengetahui (kiri) & Mengetahui/Menyetujui (tengah)
  MRG(1, sigLabel + 3, 2, sigLabel + 3); ctrw(setv(1, sigLabel + 3, 'Mengetahui', ar10));
  MRG(4, sigLabel + 3, 5, sigLabel + 3); ctr(setv(4, sigLabel + 3, 'Mengetahui/Menyetujui', cal11));
  // Jabaran di-MERGE & center (revisi user) -- dulu merge-nya tidak sengaja
  // hilang saat edit rata-atas. Tinggi merge mengikuti teks: Supervisor 3 baris
  // (A..B, sigLabel+4..sigLabel+6), Manajer Produksi 1 baris (D..E). Teks tetap
  // rata-ATAS supaya tidak ada ruang kosong di bawah teks pendek.
  MRG(1, sigLabel + 4, 2, sigLabel + 6);
  const roleLeftCell = ctrw(setv(1, sigLabel + 4, meta.roleMengetahui || '', cal12));
  roleLeftCell.alignment.vertical = 'top';
  MRG(4, sigLabel + 4, 5, sigLabel + 4);
  const roleMidCell = ctrw(setv(4, sigLabel + 4, meta.roleMenyetujui || '', cal11));
  roleMidCell.alignment.vertical = 'top';
  MRG(1, nameRow, 2, nameRow); ctrw(setv(1, nameRow, meta.nameMengetahui || '', ar10b));
  MRG(4, nameRow, 5, nameRow); ctrw(setv(4, nameRow, meta.nameMenyetujui || '', ar10b));

  // Kode dokumen (kotak tipis, kiri bawah) -- di-MERGE A:B & center (revisi
  // user). Border di-set di sel MASTER merge supaya kotak A:B utuh, garis atas
  // lanjut ke kolom C (pola contoh).
  MRG(1, codeRow, 2, codeRow);
  ctr(setv(1, codeRow, meta.footerCode || '', ar10b));
  bdAdd(1, codeRow, 'tlbr', THIN);
  bdAdd(3, codeRow, 't', THIN);

  // Tinggi baris seragam persis contoh (15.75).
  for (let r = 1; r <= codeRow; r++) ws.getRow(r).height = 15.75;

  return wb.xlsx.writeBuffer();
}

// 9.8 Laporan Jadwal Kalibrasi (apps/kpi-sab/kalibrasi-9-8.html).
//
// SATU sheet "9" per bulan. SEMUA isian MANUAL oleh admin (pola 9.4/9.5,
// tidak ada data otomatis & tidak ada kunci akses): daftar alat yang
// dikalibrasi (kpi_9_8_items, rows JSONB per bulan) + penandatangan 2 kolom
// (Mengetahui/Menyetujui [kiri] + Dibuat oleh [kanan]), kode dokumen
// "PTMBPP-IR-PRD.SAB/01-08", Lokasi & Halaman (kpi_9_8_meta, global).
// Unduhan Excel persis contoh "9.8 Laporan Jadwal Kalibrasi.xlsx".
// ===========================================================================

const K98_META_KEY = 'global';
const K98_DEFAULT_META = {
  roleMenyetujui: 'Manajer Produksi',
  nameMenyetujui: 'DEDY HERMAWAN S.M',
  roleDibuat: 'Supervisor Sumber Air Baku & Lingkungan',
  nameDibuat: 'DARTO',
  footerCode: 'PTMBPP-IR-PRD.SAB/01-08',
  halaman: '1',
  totalHalaman: '1',
  lokasi: 'Waduk Manggar'
};

async function loadGlobalK98Meta() {
  const { rows } = await pool.query('SELECT * FROM kpi_9_8_meta WHERE period_key = $1', [K98_META_KEY]);
  if (!rows[0]) return null;
  const r = rows[0];
  return {
    roleMenyetujui: r.role_menyetujui || '',
    nameMenyetujui: r.name_menyetujui || '',
    roleDibuat: r.role_dibuat || '',
    nameDibuat: r.name_dibuat || '',
    footerCode: r.footer_code || '',
    halaman: r.halaman || '',
    totalHalaman: r.total_halaman || '',
    lokasi: r.lokasi || ''
...
async function saveK98Meta(m) {
  await pool.query(
    `INSERT INTO kpi_9_8_meta (period_key, role_menyetujui, name_menyetujui, role_dibuat, name_dibuat, footer_code, total_halaman, halaman, lokasi, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, now())
     ON CONFLICT (period_key) DO UPDATE SET
       role_menyetujui = EXCLUDED.role_menyetujui, name_menyetujui = EXCLUDED.name_menyetujui,
       role_dibuat = EXCLUDED.role_dibuat, name_dibuat = EXCLUDED.name_dibuat,
       footer_code = EXCLUDED.footer_code, total_halaman = EXCLUDED.total_halaman,
       halaman = EXCLUDED.halaman, lokasi = EXCLUDED.lokasi,
       updated_at = now()`,
    [K98_META_KEY, m.roleMenyetujui || '', m.nameMenyetujui || '', m.roleDibuat || '', m.nameDibuat || '', m.footerCode || '', m.totalHalaman || '', m.halaman || '', m.lokasi || '']
  );
}

async function loadK98Items(bulan) {
  const { rows } = await pool.query('SELECT rows FROM kpi_9_8_items WHERE bulan = $1', [bulan + '-01']);
  return rows[0] ? rows[0].rows : [];
}

async function saveK98Items(bulan, items) {
  const clean = (items || []).map(r => ({
    no: (r && r.no !== null && r.no !== undefined) ? String(r.no) : '',
    namaAlat: (r && r.namaAlat !== null && r.namaAlat !== undefined) ? String(r.namaAlat) : '',
    noSeri: (r && r.noSeri !== null && r.noSeri !== undefined) ? String(r.noSeri) : '',
    kodeInventaris: (r && r.kodeInventaris !== null && r.kodeInventaris !== undefined) ? String(r.kodeInventaris) : '',
    terakhir: (r && r.terakhir !== null && r.terakhir !== undefined) ? String(r.terakhir) : '',
    berikutnya: (r && r.berikutnya !== null && r.berikutnya !== undefined) ? String(r.berikutnya) : '',
    keterangan: (r && r.keterangan !== null && r.keterangan !== undefined) ? String(r.keterangan) : ''
  }));
  await pool.query(
    `INSERT INTO kpi_9_8_items (bulan, rows, updated_at) VALUES ($1, $2::jsonb, now())
     ON CONFLICT (bulan) DO UPDATE SET rows = EXCLUDED.rows, updated_at = now()`,
    [bulan + '-01', JSON.stringify(clean)]
  );
}

async function getKpi9_8Data(access, bulanQuery) {
  const currentYm = currentYmWITA();
  let bulan = (typeof bulanQuery === 'string' && /^\d{4}-\d{2}$/.test(bulanQuery)) ? bulanQuery : null;
  // Default: bulan terakhir yang sudah diisi admin (kalau ada & dalam rentang).
  if (!bulan) {
    const latest = await latestDataYm([{ table: 'kpi_9_8_items', monthCol: 'bulan' }]);
    if (latest && ymToIndex(latest) >= ymToIndex(KUALITAS_START_YM)) bulan = latest;
  }
  if (!bulan) bulan = currentYm;
  const availableMonths = buildAvailableMonths(bulan > currentYm ? bulan : currentYm);
  ...
  const year = Number(yStr), monthIndex = Number(mStr) - 1;
  const monthTitle = `${MONTHS_TITLE[monthIndex]} ${year}`;
  const items = await loadK98Items(bulan);
  const meta = Object.assign({}, K98_DEFAULT_META, (await loadGlobalK98Meta()) || {});
  return { locked: false, availableMonths, bulan, year, monthIndex, monthTitle, items, meta };
}

// --- Unduh Excel 9.8 -- exceljs di server. Meniru PERSIS file contoh
// "9.8 Laporan Jadwal Kalibrasi.xlsx" (sheet "9"): kop perusahaan, "LAMPIRAN :
// 9.8 LAPORAN JADWAL KALIBRASI", Nomor/No.Revisi/Tgl.Revisi + judul + Hal,
// Bagian/Lokasi/Bulan, header NO | NAMA ALAT | NO.SERI | KODE INVENTARIS |
// KALIBRASI EXTERNAL (TERAKHIR | BERIKUTNYA) | KETERANGAN, baris data (border
// HAIR horisontal + THIN vertikal, KETERANGAN di-merge G:H per baris), tabel
// setinggi minimal 21 baris, penandatangan 2 kolom & kode dokumen di footer.
const K98_HAIR = { style: 'hair', color: { argb: 'FF000000' } };
const K98_COL_WIDTHS = [7.44, 21.44, 11.55, 12.89, 16.89, 18.11, 9.11, 9.11];

function buildKpi9_8ExcelWorkbook(data) {
  const wb = new ExcelJS.Workbook();
  wb.calcProperties.fullCalcOnLoad = true;
  const ws = wb.addWorksheet('9', {
    pageSetup: { orientation: 'portrait', paperSize: 5, fitToPage: true, fitToWidth: 1, fitToHeight: 1, blackAndWhite: false, draft: false }
  });
  ws.pageSetup.margins = { left: 0.95, right: 0.7, top: 1.25, bottom: 0.75, header: 0, footer: 0 };
  K98_COL_WIDTHS.forEach((w, i) => { ws.getColumn(i + 1).width = w; });
  ...
  const cal11 = { name: 'Calibri', size: 11 };
  const cal11b = { name: 'Calibri', size: 11, bold: true };
  const cal10 = { name: 'Calibri', size: 10 };

  const meta = data.meta || {};
  const hal = meta.halaman || '1';
  const totalHal = meta.totalHalaman || '1';
  const lokasi = meta.lokasi || 'Waduk Manggar';
  const items = data.items || [];

  // --- Kop (baris 3-11) ---
  MRG(1, 3, 8, 3);
  setv(1, 3, 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG KOTA BALIKPAPAN', ar10);
  for (let c = 1; c <= 8; c++) bdT(c, 3, 'tb');
  bdT(1, 3, 'l'); bdT(8, 3, 'r');

  MRG(1, 4, 8, 5);
  setv(1, 4, 'LAMPIRAN : 9.8 LAPORAN JADWAL KALIBRASI', cal11);
  bdT(1, 4, 'tl'); for (let c = 2; c <= 8; c++) bdT(c, 4, 't');
  bdT(1, 5, 'lb'); for (let c = 2; c <= 8; c++) bdT(c, 5, 'b');
  bdT(8, 4, 'tr'); bdT(8, 5, 'br');

  // Nomor / No.Revisi / Tgl.Revisi (kiri) + judul + Hal (kanan)
  MRG(1, 6, 2, 6); setv(1, 6, 'Nomor        :', cal11);
  MRG(1, 7, 2, 7); setv(1, 7, 'No.Revisi   :', cal11);
  MRG(1, 8, 2, 8); setv(1, 8, 'Tgl.Revisi   :', cal11);
  for (let c = 1; c <= 2; c++) bdT(c, 6, 't');
  for (let c = 1; c <= 2; c++) bdT(c, 8, 'b');
  for (let r = 6; r <= 8; r++) { bdT(1, r, 'l'); bdT(2, r, 'r'); }
  MRG(4, 6, 8, 7);
  ctrw(setv(4, 6, 'LAPORAN JADWAL KALIBRASI', cal11));
  for (let c = 4; c <= 8; c++) bdT(c, 6, 't');
  for (let c = 4; c <= 8; c++) bdT(c, 7, 'b');
  bdT(4, 6, 'l'); bdT(8, 6, 'r'); bdT(4, 7, 'l'); bdT(8, 7, 'r');
  MRG(4, 8, 8, 8);
  ctr(setv(4, 8, 'Hal  :  ' + hal + '  Dari  :  ' + totalHal, cal11));
  for (let c = 4; c <= 8; c++) bdT(c, 8, 'tb');
  bdT(4, 8, 'l'); bdT(8, 8, 'r');

  // Bagian / Lokasi / Bulan (nilai di-merge B:F, kolom G terpisah; box tepi
  // kanan di kolom H seperti contoh)
  setv(1, 9, 'Bagian', cal11); MRG(2, 9, 6, 9); setv(2, 9, ': Sumber Air Baku', cal11);
  setv(1, 10, 'Lokasi', cal11); MRG(2, 10, 6, 10); setv(2, 10, ': ' + lokasi, cal11);
  setv(1, 11, 'Bulan', cal11); MRG(2, 11, 6, 11); setv(2, 11, ': ' + data.monthTitle, ar10b);
  for (let c = 1; c <= 8; c++) bdT(c, 9, 't');
  for (let c = 1; c <= 8; c++) bdT(c, 11, 'b');
  for (let r = 9; r <= 11; r++) { bdT(1, r, 'l'); bdT(8, r, 'r'); }

  // --- Header tabel (baris 12-13) ---
  MRG(1, 12, 1, 13); ctrw(setv(1, 12, 'NO', cal11));
  MRG(2, 12, 2, 13); ctrw(setv(2, 12, 'NAMA ALAT', cal11));
  MRG(3, 12, 3, 13); ctrw(setv(3, 12, 'NO.SERI', cal11));
  MRG(4, 12, 4, 13); ctrw(setv(4, 12, 'KODE INVENTARIS', cal11));
  MRG(5, 12, 6, 12); ctrw(setv(5, 12, 'KALIBRASI EXTERNAL', cal11));
  ctrw(setv(5, 13, 'TERAKHIR', cal11));
  ctrw(setv(6, 13, 'BERIKUTNYA', cal11));
  MRG(7, 12, 8, 13); ctrw(setv(7, 12, 'KETERANGAN', cal11));
  // Border header: di-set di SEMUA sel (termasuk slave merge) supaya garis
  // atas KETERANGAN & tepi header render utuh; bawah baris 13 HAIR (pola
  // contoh), kotak KALIBRASI EXTERNAL (E12:F12) garis bawah tipis.
  for (let r = 12; r <= 13; r++) {
    for (let c = 1; c <= 8; c++) {
      const b = CELL(c, r).border || {};
      b.top = THIN; b.left = THIN; b.right = THIN;
      CELL(c, r).border = b;
    }
  }
  for (let c = 1; c <= 8; c++) bdAdd(c, 13, 'b', K98_HAIR);
  bdT(5, 12, 'b'); // KALIBRASI EXTERNAL (E12:F12) garis bawah tipis

  // --- Baris data (14..14+tableHeight-1, minimal 23 baris seperti contoh).
  // Baris TERAKHIR diberi border bawah THIN sebagai penutup isi data. ---
  const tableHeight = Math.max(23, items.length);
  for (let i = 0; i < tableHeight; i++) {
    ...
    for (let c = 1; c <= 8; c++) {
      const b = CELL(c, r).border || {};
      b.top = K98_HAIR; b.left = THIN; b.right = THIN;
      b.bottom = (i === tableHeight - 1) ? THIN : K98_HAIR;
      CELL(c, r).border = b;
    }
    if (item) {
      ctrw(setv(1, r, item.no, cal11));
      ctrw(setv(2, r, item.namaAlat, cal10));
      ctrw(setv(3, r, item.noSeri, cal10));
      ctrw(setv(4, r, item.kodeInventaris, cal11));
      ctr(setv(5, r, item.terakhir, cal10));
      ctr(setv(6, r, item.berikutnya, cal10));
      const kc = setv(7, r, item.keterangan, cal10);
      if (kc) kc.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
    }
  }

  // --- Penandatangan 2 kolom + kode dokumen ---
  const sigOffset = tableHeight - 23;
  const sigLabel = 39 + sigOffset; // Mengetahui/Menyetujui + Dibuat oleh
  const sigRole = 40 + sigOffset;
  const nameLeft = 44 + sigOffset;  // DEDY (Mengetahui/Menyetujui)
  const nameRight = 45 + sigOffset; // DARTO (Dibuat oleh)
  const codeRow = 48 + sigOffset;

  setv(2, sigLabel, 'Mengetahui/Menyetujui', cal11);
  setv(2, sigRole, meta.roleMenyetujui || '', cal11);
  MRG(1, nameLeft, 3, nameLeft); ctrw(setv(1, nameLeft, meta.nameMenyetujui || '', cal11b));
  MRG(5, sigLabel, 8, sigLabel); ctr(setv(5, sigLabel, 'Dibuat oleh', cal11));
  MRG(5, sigRole, 8, sigRole); ctrw(setv(5, sigRole, meta.roleDibuat || '', cal11));
  MRG(5, nameRight, 8, nameRight); ctrw(setv(5, nameRight, meta.nameDibuat || '', cal11b));
  // Kode dokumen dengan kotak border tipis (pola contoh A48:B48).
  MRG(1, codeRow, 2, codeRow); setv(1, codeRow, meta.footerCode || '', cal11b);
  bdT(1, codeRow, 'tlb'); bdT(2, codeRow, 'tbr');

  // Tinggi baris seragam persis contoh (15.75).
  for (let r = 1; r <= codeRow; r++) ws.getRow(r).height = 15.75;
  // Baris 14 (baris data pertama) diperbesar ke 35px (26.25pt) -- revisi user.
  ws.getRow(14).height = 26.25;
...
module.exports = {
  ...
  getKpi9_4Data, saveK94Meta, saveK94Items, buildKpi9_4ExcelWorkbook,
  getKpi9_7Data, saveK97Meta, saveK97Items, buildKpi9_7ExcelWorkbook,
  getKpi9_5Data, saveK95Meta, saveK95Items, buildKpi9_5ExcelWorkbook,
  getKpi9_8Data, saveK98Meta, saveK98Items, buildKpi9_8ExcelWorkbook
};

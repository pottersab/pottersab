...
async function getKpi192Data(access, bulanQuery) {
  ...
  const availableMonths = Array.from(new Set([...monthsRows.map(r => r.ym), currentYm])).sort();
  if (!bulan || !availableMonths.includes(bulan)) bulan = availableMonths[availableMonths.length - 1];
...
const KPI192_COL_WIDTHS = [6.33, 6.66, 6.66, 8.33, 8.66, 13.55, 8.66, 11, 8.66, 8.66, 8.66, 8.66, 8.66, 76];
...
function buildKpi192ExcelWorkbook(data) {
  const wb = new ExcelJS.Workbook();
  wb.calcProperties.fullCalcOnLoad = true;
  const ws = wb.addWorksheet('19.2 EVALUASI HASIL MONITORING', { pageSetup: { orientation: 'landscape', fitToPage: true, paperSize: 5 } });
  KPI192_COL_WIDTHS.forEach((w, i) => { ws.getColumn(i + 1).width = w; });

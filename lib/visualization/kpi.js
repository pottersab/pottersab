...
//   6. Pengambilan Sample air waduk         -- tanpa tanda (sel tanggal di-merge)
...
const JK_KEGIATAN = [
  { no: 1, uraian: 'Monitoring Level waduk/Curah Hujan', rule: 'harian' },
  { no: 2, uraian: 'Monitoring Level dan Debit sumur', rule: 'sampai15' },
  { no: 3, uraian: 'Monitoring Kualitas air baku', rule: 'harian' },
  { no: 4, uraian: 'Survey Pipa Transmisi', rule: 'survey' },
  { no: 5, uraian: 'Survey Pipa Transmisi (sumur sumur)', rule: 'survey' },
  { no: 6, uraian: 'Pengambilan Sample air waduk', rule: 'sampling' }
];
...
// hari di baris 8, nomor tanggal di baris 9), 6 baris kegiatan dengan "√",
// satu baris pemisah kosong ber-garis, blok tanda tangan 3 kolom (Mengetahui/
// Menyetujui | Diketahui/Disetujui | Dibuat Oleh), lalu kode dokumen footer.
  ...
  data.rows.forEach((row, ri) => {
    ...
    if (row.merged) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (row.cells[i]) { cell.value = row.cells[i]; cell.alignment = { horizontal: 'center' }; }

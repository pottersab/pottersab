...
async function getKpi9_2Data(access, bulanQuery) {
  ...
  const locations = KUALITAS_LOCATIONS.map(loc => buildLocationMonth(
    loc,
...
const K92_PENGURANGAN = 17.4; // pengurangan untuk Level Waduk Teritip (N = X - Y)
const K92_COL_WIDTHS = [11, 8.44, 9.66, 8.66, 8.66, 10.33, 10.44, 8.66, 8.66, 5.11, 10.66, 8.66, 12, 9.44, 8.66, 8.66, 11, 8.66, 8.66, 8.66, 8.66, 8.66, 8.66, 8.66, 15.44];
...
function buildKpi9_2ExcelWorkbook(data) {
  ...
  // Blok kiri = Manggar, kanan = Teritip (urutan KUALITAS_LOCATIONS)
  ...
  // Kolom bantu X (mdpl) & Y (Pengurangan) untuk Level Teritip = X - Y.
  // Data level Teritip = N; X = N + pengurangan, Y = pengurangan.
  const teritip = data.locations[1];
  const terDays = teritip.rows.length;
  ctr(setv(24, 10, 'mdpl', {}));
  ctr(setv(25, 10, 'Pengurangan', {}));
  for (let r = 11; r <= 41; r++) {
    const di = r - 11;
    const row = teritip.rows[di] || null;
    const lvl = (row && row.level !== null) ? row.level : null;
    if (lvl !== null) {
      setv(24, r, Math.round((lvl + K92_PENGURANGAN) * 100) / 100, {});
      setv(25, r, K92_PENGURANGAN, {});
      const cellN = CELL(14, r);
      cellN.value = { formula: `X${r}-Y${r}`, result: lvl };
      cellN.font = { name: 'Arial', size: 11 };

...
// di columns.js, lewat fetchWideSingleRows) -- laporan ini cuma merata-ratakan
// per bulan (avgWideSingleByMonth), TIDAK menyimpan angka sendiri.
//
// Yang benar-benar baru cuma status ON/OFF pintu elevasi 3/5/7 per hari per
// lokasi (kualitas_pintu_elevasi) -- ada kasus operator harus buka pintu
// elevasi lebih rendah (mis. elevasi 5) kalau air di elevasi atas keruh, jadi
// BISA lebih dari satu elevasi ON bersamaan (bukan pilihan tunggal). Diedit
// ADMIN langsung di halaman ini lewat panel harian di bawah tabel utama
// (pilih bulan, klik toggle per elevasi per hari) -- BUKAN lewat Input Massal
// seperti Level/NTU/PH, karena ini status operasional, bukan hasil ukur.
// ===========================================================================
...
// di bulan itu, hasilnya null (bukan 0).
...
// Jumlah hari ON per bulan (0-11) untuk satu lokasi+elevasi, dari baris
// kualitas_pintu_elevasi tahun yang sedang dilihat.
function elevasiOnDaysByMonth(elevasiRows, locKey, elevasi, year) {
  const counts = new Array(12).fill(0);
  elevasiRows.forEach(r => {
...
// Baris cuma ada untuk tanggal yang PERNAH disentuh -- tanggal yang tidak
// muncul di hasil query dianggap semua elevasi OFF oleh pemanggil (client
// & elevasiOnDaysByMonth di atas sama-sama menganggap "tidak ada baris" =
// false, bukan error).
async function loadKualitasElevasiRows(year) {
  const { rows } = await pool.query(
    `SELECT to_char(tanggal, 'YYYY-MM-DD') as d,
            manggar_e3, manggar_e5, manggar_e7, teritip_e3, teritip_e5, teritip_e7
     FROM kualitas_pintu_elevasi
  ...
  return rows.map(r => ({
    Tanggal: r.d,
    manggar: { 3: !!r.manggar_e3, 5: !!r.manggar_e5, 7: !!r.manggar_e7 },
    teritip: { 3: !!r.teritip_e3, 5: !!r.teritip_e5, 7: !!r.teritip_e7 }
  }));
}
...
async function saveKualitasElevasi(tanggal, loc, elevasi, on) {
  ...
  const col = `${loc}_e${e}`;
  await pool.query(
    `INSERT INTO kualitas_pintu_elevasi (tanggal, ${col}) VALUES ($1, $2)
     ON CONFLICT (tanggal) DO UPDATE SET ${col} = EXCLUDED.${col}`,
    [tanggal, !!on]
  );
}
...
async function getKpiKualitasData(access, tahunQuery) {
  if (!access.granted) {
    ...
    // asli: elevasi 3 OFF, elevasi 5 & 7 ON sepanjang bulan.
    const year = tahunQuery || String(new Date().getFullYear());
  ...
  const elevasiRows = await loadKualitasElevasiRows(year);
  ...
  const groups = KUALITAS_LOCATIONS.map(loc => {
    ...
    return {
      ...
      elevasiHari: {
        3: elevasiOnDaysByMonth(elevasiRows, loc.key, 3, year),
        5: elevasiOnDaysByMonth(elevasiRows, loc.key, 5, year),
        7: elevasiOnDaysByMonth(elevasiRows, loc.key, 7, year)
      }
    };
  });
  ...
  return { locked: false, availableYears, year, groups, elevasiDaily: elevasiRows, meta };
}

// --- Unduh Excel 18.4 Laporan Kualitas Air Baku -- exceljs di server, alasan
// sama persis seperti KPI lain di atas. Formatnya SEDERHANA dibanding lampiran
// Excel asli (yang berbentuk blok harian 1-31 per bulan) -- mengikuti bentuk
// tabel tahunan (baris x kolom bulan) yang sudah dipakai 18.2/18.3a/18.3b di
// web, supaya konsisten dengan tampilan halaman ini. Hari ON per elevasi
// ditampilkan sebagai ringkasan bulanan (jumlah hari), bukan status harian --
// status harian tetap bisa dilihat/diedit di panel bawah halaman web. ---
const KUALITAS_COL_WIDTHS = [8.66, 30, 14, 14, 14, 14, 14, 14];

function buildKualitasSubTable(ws, r, judulBulan, group, monthBase) {
  ws.mergeCells(`A${r}:H${r}`);
  ws.getCell(`A${r}`).value = `18.4 LAPORAN KUALITAS AIR BAKU — ${group.label.toUpperCase()}`;
  ws.getCell(`A${r}`).font = { bold: true, size: 13, name: TNR };
  ws.getCell(`A${r}`).alignment = { horizontal: 'center' };
  r++;
  ...
  const headRow = r;
  ws.getCell(`A${headRow}`).value = 'NO';
  ws.getCell(`B${headRow}`).value = 'URAIAN';
  MONTHS_ID.slice(monthBase, monthBase + 6).forEach((mn, i) => {
    ws.getCell(`${colLetter(3 + i)}${headRow}`).value = mn;
  });
  for (let c = 1; c <= 8; c++) {
    const cell = ws.getRow(headRow).getCell(c);
    cell.font = { bold: true, size: 11, name: TNR };
    cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
    cell.border = GRID;
  }
  ...
  const ITEMS = [
    { label: 'Level Waduk (m)', values: group.level, fmt: '0.00' },
    { label: 'Kekeruhan / NTU (rata-rata)', values: group.ntu, fmt: '0.0' },
    { label: 'PH Air Baku (rata-rata)', values: group.ph, fmt: '0.00' },
    { label: 'Elevasi 3 — Hari Pintu Dibuka (ON)', values: group.elevasiHari[3], fmt: '0' },
    { label: 'Elevasi 5 — Hari Pintu Dibuka (ON)', values: group.elevasiHari[5], fmt: '0' },
    { label: 'Elevasi 7 — Hari Pintu Dibuka (ON)', values: group.elevasiHari[7], fmt: '0' }
  ];
...
function buildKualitasSignBlock(ws, r, meta) {
  const ketHeaderRow = r;
  ws.getCell(`B${ketHeaderRow}`).value = 'Keterangan :';
  ws.getCell(`B${ketHeaderRow}`).font = { size: 11, name: TNR };
  r++;
  ...
  ws.mergeCells(`G${labelRow}:H${labelRow}`);
  ws.getCell(`G${labelRow}`).value = KUALITAS_LABEL_RIGHT;
  ws.getCell(`G${labelRow}`).font = { size: 11, name: TNR };
  ws.getCell(`G${labelRow}`).alignment = { horizontal: 'center' };
...
function buildKualitasExcelBlock(ws, rowOffset, judulBulan, data, monthBase) {
  const R = r => r + rowOffset;
  ws.getCell(`A${R(1)}`).value = 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG';
  ws.getCell(`A${R(1)}`).font = { bold: true, size: 12, name: 'Arial' };
  ws.getCell(`A${R(2)}`).value = 'KOTA BALIKPAPAN';
  ...
  let r = R(4);
  data.groups.forEach(g => {
    r = buildKualitasSubTable(ws, r, judulBulan, g, monthBase);
    r++;
  });
  return buildKualitasSignBlock(ws, r, data.meta);
}
...
async function buildKpiKualitasExcelWorkbook(data) {
  ...
  const nextRow = buildKualitasExcelBlock(ws, 0, `BULAN : Januari - Juni ${data.year}`, data, 0);
  const rowOffset2 = nextRow + 1;
  buildKualitasExcelBlock(ws, rowOffset2, `BULAN : Juli - Desember ${data.year}`, data, 6);

...
function buildKpi9_8ExcelWorkbook(data) {
  ...
  // --- Baris data (14..14+tableHeight-1, minimal 21 baris seperti contoh) ---
  const tableHeight = Math.max(21, items.length);
  for (let i = 0; i < tableHeight; i++) {
    ...
    for (let c = 1; c <= 8; c++) {
      const b = CELL(c, r).border || {};
      b.top = K98_HAIR; b.left = THIN; b.bottom = K98_HAIR; b.right = THIN;
      CELL(c, r).border = b;
    }
    if (item) {
  ...
  // --- Penandatangan 2 kolom + kode dokumen ---
  const sigOffset = tableHeight - 21;

...
const KUAL_NOTE = 'Data diambil dari IPA 1 setiap jam 8.00';
...
function buildKualitasSheet(wb, loc, monthTitle, meta) {
  ...
  // Judul (baris 4) lalu BULAN (baris 5) -- urutan sesuai file revisi.
  ws.mergeCells('A4:H4');
  ws.getCell('A4').value = loc.title;
  ws.getCell('A4').font = kualCellFont({ bold: true, size: 11 });
  ws.getCell('A4').alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.mergeCells('A5:H5');
  ws.getCell('A5').value = `BULAN : ${monthTitle}`;
  ws.getCell('A5').font = kualCellFont({ bold: true, size: 11 });
  ...
  // Kolom KETERANGAN (H): satu catatan pengambilan sampel, di-merge sepanjang
  // baris data supaya rapi (sesuai file revisi yang menaruh note di kolom ini).
  if (lastDataRow >= firstDataRow) {
    ws.mergeCells(`${c(7)}${firstDataRow}:${c(7)}${lastDataRow}`);
    const noteCell = ws.getCell(`${c(7)}${firstDataRow}`);
    noteCell.value = KUAL_NOTE;
    noteCell.font = kualCellFont();
    noteCell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  }

  // Ringkasan: Rata rata / Tertinggi / Terendah (Level di B, NTU di F, PH di G)
  ...
  // Tanda tangan (Keterangan list kiri, blok tanda tangan)
  r += 1;
  const ketHeaderRow = r;

...
const KUALITAS_LOCATIONS = [
  { key: 'manggar', label: 'Waduk Manggar', levelDs: 'manggar_level', ntuDs: 'manggar_ntu', phDs: 'manggar_ph' },
  { key: 'teritip', label: 'Waduk Teritip', levelDs: 'teritip_level', ntuDs: 'teritip_ntu', phDs: 'teritip_ph' }
];

function pad2(n) { return n < 10 ? '0' + n : '' + n; }

// Rata-rata per bulan (0-11) dari baris 'wide-single' harian (bentuk
// { Tanggal:'YYYY-MM-DD', <csvCol>: n, ... }) -- beda dari sumRealByMonth di
// atas yang MENJUMLAH baris 'wide' bulanan. Kalau tidak ada satu pun bacaan
// di bulan itu, hasilnya null (bukan 0). Dipakai tabel ringkasan bulanan
// di web.
function avgWideSingleByMonth(rows, dateKey, valueKey, year) {
  ...
  return sums.map((s, i) => (counts[i] > 0 ? Math.round((s / counts[i]) * 100) / 100 : null));
}

// Peta tanggal -> nilai (bukan rata-rata) untuk SATU tahun -- dipakai unduhan
// Excel yang butuh angka harian persis seperti di web (Data & Visualisasi),
// bukan rata-rata bulanan.
function wideSingleDateMap(rows, dateKey, valueKey, year) {
  const map = {};
  rows.forEach(r => {
    const d = r[dateKey];
    if (!d || d.slice(0, 4) !== year) return;
...
// Jumlah hari ON per bulan (0-11) untuk satu lokasi+elevasi, dari elevasiDaily
// (array HASIL forward-fill buildElevasiDailyForYear -- selalu lengkap 1
// baris per tanggal dalam tahun itu, bukan cuma tanggal yang disentuh).
function elevasiOnDaysByMonth(elevasiDaily, locKey, elevasi, year) {
  const counts = new Array(12).fill(0);
  elevasiDaily.forEach(r => {
    if (!r.Tanggal || !r.Tanggal.startsWith(year)) return;
    if (r[locKey][elevasi]) counts[Number(r.Tanggal.slice(5, 7)) - 1]++;
  });
  return counts;
}

// Ambil semua log perubahan status sampai tanggal tertentu (ASC) -- dipakai
// buildElevasiDailyForYear untuk forward-fill, termasuk perubahan dari
// tahun-tahun SEBELUMNYA (supaya status yang di-ON kan tahun lalu tetap
// kebaca ON di awal tahun ini kalau belum pernah di-OFF kan lagi).
...
// Forward-fill: tiap tanggal dalam `year` dapat status elevasi TERAKHIR yang
// berlaku sampai tanggal itu (bisa berasal dari tahun sebelumnya kalau belum
// pernah diubah lagi tahun ini) -- BUKAN dihitung ulang dari nol tiap bulan.
// Hasilnya selalu lengkap 365/366 baris (satu per tanggal), beda dari
// `events` yang cuma berisi baris PERUBAHAN.
function buildElevasiDailyForYear(events, year) {
  const state = {
    manggar: { 3: false, 5: false, 7: false },
    teritip: { 3: false, 5: false, 7: false }
  };
  const yearStart = `${year}-01-01`;
  const byDate = new Map();
  events.forEach(e => {
    if (e.tanggal_mulai < yearStart) {
...
// Peta tanggal -> baris elevasi harian, dibangun sekali dari elevasiDaily
// (array) supaya pencarian per tanggal di unduhan Excel O(1), bukan scan
// array tiap sel.
function elevasiDailyToMap(elevasiDaily) {
  const map = {};
  (elevasiDaily || []).forEach(r => { map[r.Tanggal] = r; });
  return map;
}
...
async function getKpiKualitasData(access, tahunQuery) {
  if (!access.granted) {
    // Sama seperti KPI lain -- kalau belum ada akses, Level/NTU/PH dummy
    // (skala acak dari dummyMax dataset asli). Status pintu elevasi dummy
    // dibuat TETAP (bukan disimpan/diedit) mengikuti pola paling umum di data
    // asli: elevasi 3 OFF, elevasi 5 & 7 ON sepanjang tahun.
    const year = tahunQuery || String(new Date().getFullYear());
    const dailyByLocation = {};
    const groups = KUALITAS_LOCATIONS.map(loc => {
      ...
      dailyByLocation[loc.key] = {
        level: wideSingleDateMap(levelDummy.rows, levelDummy.dateKey, DATASETS[loc.levelDs].csvCol, year),
        ntu: wideSingleDateMap(ntuDummy.rows, ntuDummy.dateKey, DATASETS[loc.ntuDs].csvCol, year),
        ph: wideSingleDateMap(phDummy.rows, phDummy.dateKey, DATASETS[loc.phDs].csvCol, year)
      };
      return {
        key: loc.key,
        label: loc.label,
        level: avgWideSingleByMonth(levelDummy.rows, levelDummy.dateKey, DATASETS[loc.levelDs].csvCol, year),
        ntu: avgWideSingleByMonth(ntuDummy.rows, ntuDummy.dateKey, DATASETS[loc.ntuDs].csvCol, year),
        ph: avgWideSingleByMonth(phDummy.rows, phDummy.dateKey, DATASETS[loc.phDs].csvCol, year),
        elevasiHari: { 3: new Array(12).fill(0), 5: new Array(12).fill(28), 7: new Array(12).fill(28) }
      };
    });
    const dummyEvents = [];
    ['manggar', 'teritip'].forEach(lokasi => {
      [5, 7].forEach(elevasi => dummyEvents.push({ lokasi, elevasi, tanggal_mulai: `${Number(year) - 5}-01-01`, status: true }));
    });
    const elevasiDaily = buildElevasiDailyForYear(dummyEvents, year);
    return {
      locked: true,
      availableYears: [],
      year,
  ...
  const availableYearsSet = new Set();
  dsKeys.forEach(k => {
    const { dateKey, rows } = fetched[k];
    rows.forEach(r => { if (r[dateKey]) availableYearsSet.add(r[dateKey].slice(0, 4)); });
  });
  const availableYears = Array.from(availableYearsSet).sort();
  const currentRealYear = String(new Date().getFullYear());
  if (!availableYears.includes(currentRealYear)) availableYears.push(currentRealYear);
  ...
  const events = await loadKualitasElevasiEvents(`${year}-12-31`);
  const elevasiDaily = buildElevasiDailyForYear(events, year);

  const dailyByLocation = {};
  const groups = KUALITAS_LOCATIONS.map(loc => {
    const levelData = fetched[loc.levelDs], ntuData = fetched[loc.ntuDs], phData = fetched[loc.phDs];
    dailyByLocation[loc.key] = {
      level: wideSingleDateMap(levelData.rows, levelData.dateKey, DATASETS[loc.levelDs].csvCol, year),
      ntu: wideSingleDateMap(ntuData.rows, ntuData.dateKey, DATASETS[loc.ntuDs].csvCol, year),
      ph: wideSingleDateMap(phData.rows, phData.dateKey, DATASETS[loc.phDs].csvCol, year)
    };
    return {
      key: loc.key,
      label: loc.label,
      level: avgWideSingleByMonth(levelData.rows, levelData.dateKey, DATASETS[loc.levelDs].csvCol, year),
      ntu: avgWideSingleByMonth(ntuData.rows, ntuData.dateKey, DATASETS[loc.ntuDs].csvCol, year),
      ph: avgWideSingleByMonth(phData.rows, phData.dateKey, DATASETS[loc.phDs].csvCol, year),
      elevasiHari: {
        3: elevasiOnDaysByMonth(elevasiDaily, loc.key, 3, year),
        5: elevasiOnDaysByMonth(elevasiDaily, loc.key, 5, year),
        7: elevasiOnDaysByMonth(elevasiDaily, loc.key, 7, year)
      }
    };
  });
  ...
  return { locked: false, availableYears, year, groups, dailyByLocation, elevasiDaily, meta };
}

// --- Unduh Excel 18.4 Laporan Kualitas Air Baku -- exceljs di server, alasan
// sama persis seperti KPI lain di atas. BEDA dari 18.2/18.3a/18.3b: formatnya
// meniru PERSIS lampiran Excel asli (18.4 Laporan Kualitas Air Baku.xlsx) --
// 12 blok BULANAN berurutan (bukan tabel tahunan kolom-bulan), tiap blok
// punya baris HARIAN (1 s.d. jumlah hari bulan itu) dan dua lokasi
// berdampingan (Waduk Manggar kiri, Waduk Teritip kanan), ditutup baris
// RATA-RATA/TERTINGGI/TERENDAH + Keterangan & Penandatangan per blok (persis
// pola "satu halaman cetak per bulan" di file asli). ---
// 15 kolom: A-G = Waduk Manggar (TANGGAL/LEVEL/ELEV.3/ELEV.5/ELEV.7/NTU/PH),
// H = kolom pemisah kosong, I-O = Waduk Teritip (kolom sama).
const KUALITAS_COL_WIDTHS = [6.5, 9, 7.5, 7.5, 7.5, 7.5, 7.5, 3, 6.5, 9, 7.5, 7.5, 7.5, 7.5, 7.5];
const KUALITAS_LAST_COL = 15; // 'O'
const KUALITAS_GAP_COL = 8;   // 'H'

function buildKualitasSignBlockDaily(ws, r, meta) {
  const ketHeaderRow = r;
  ws.getCell(`B${ketHeaderRow}`).value = 'Keterangan :';
  ws.getCell(`B${ketHeaderRow}`).font = { size: 9, name: TNR };
  r++;
  (meta.keterangan || []).forEach(k => {
    ws.getCell(`B${r}`).value = '~ ' + k;
    ws.getCell(`B${r}`).font = { size: 9, name: TNR };
    r++;
  });
  let lastKetRow = r - 1;
  if (lastKetRow < ketHeaderRow) lastKetRow = ketHeaderRow;

  ws.mergeCells(`M${lastKetRow}:O${lastKetRow}`);
  ws.getCell(`M${lastKetRow}`).value = meta.signPlaceDate || '';
  ws.getCell(`M${lastKetRow}`).font = { size: 9, name: 'Calibri' };
  ws.getCell(`M${lastKetRow}`).alignment = { horizontal: 'center' };

  const labelRow = lastKetRow + 1;
  ws.mergeCells(`A${labelRow}:D${labelRow}`);
  ws.getCell(`A${labelRow}`).value = KUALITAS_LABEL_LEFT;
  ws.getCell(`A${labelRow}`).font = { size: 9, name: TNR };
  ws.getCell(`A${labelRow}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`M${labelRow}:O${labelRow}`);
  ws.getCell(`M${labelRow}`).value = KUALITAS_LABEL_RIGHT;
  ws.getCell(`M${labelRow}`).font = { size: 9, name: TNR };
  ws.getCell(`M${labelRow}`).alignment = { horizontal: 'center' };

  const roleRow = labelRow + 1;
  ws.mergeCells(`A${roleRow}:D${roleRow}`);
  ws.getCell(`A${roleRow}`).value = meta.roleLeft || '';
  ws.getCell(`A${roleRow}`).font = { size: 9, name: TNR };
  ws.getCell(`A${roleRow}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`M${roleRow}:O${roleRow + 1}`);
  ws.getCell(`M${roleRow}`).value = meta.roleRight || '';
  ws.getCell(`M${roleRow}`).font = { size: 9, name: TNR };
  ws.getCell(`M${roleRow}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };

  const nameRow = roleRow + 5;
  ws.mergeCells(`A${nameRow}:D${nameRow}`);
  ws.getCell(`A${nameRow}`).value = meta.nameLeft || '';
  ws.getCell(`A${nameRow}`).font = { bold: true, size: 9, name: TNR };
  ws.getCell(`A${nameRow}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`M${nameRow}:O${nameRow}`);
  ws.getCell(`M${nameRow}`).value = meta.nameRight || '';
  ws.getCell(`M${nameRow}`).font = { bold: true, size: 9, name: TNR };
  ws.getCell(`M${nameRow}`).alignment = { horizontal: 'center' };

  return nameRow + 2;
}

function buildKualitasMonthBlockExcel(ws, rowStart, year, monthIndex, dailyByLocation, elevasiDailyMap, meta) {
  let r = rowStart;

  ws.getCell(`A${r}`).value = 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG';
  ws.getCell(`A${r}`).font = { bold: true, size: 11, name: 'Arial' };
  r++;
  ws.getCell(`A${r}`).value = 'KOTA BALIKPAPAN';
  ws.getCell(`A${r}`).font = { bold: true, size: 11, name: 'Arial' };
  r += 2;

  ws.mergeCells(`A${r}:O${r}`);
  ws.getCell(`A${r}`).value = '18.4 LAPORAN KUALITAS AIR BAKU';
  ws.getCell(`A${r}`).font = { bold: true, size: 13, name: TNR };
  ws.getCell(`A${r}`).alignment = { horizontal: 'center' };
  r++;

  ws.mergeCells(`A${r}:O${r}`);
  ws.getCell(`A${r}`).value = `BULAN : ${MONTHS_ID[monthIndex]} ${year}`;
  ws.getCell(`A${r}`).font = { bold: true, size: 11, name: TNR };
  ws.getCell(`A${r}`).alignment = { horizontal: 'center' };
  r++;

  const locHeadRow = r;
  ws.mergeCells(`A${locHeadRow}:G${locHeadRow}`);
  ws.getCell(`A${locHeadRow}`).value = 'WADUK MANGGAR';
  ws.mergeCells(`I${locHeadRow}:O${locHeadRow}`);
  ws.getCell(`I${locHeadRow}`).value = 'WADUK TERITIP';
  ['A', 'I'].forEach(col => {
    const c = ws.getCell(`${col}${locHeadRow}`);
    c.font = { bold: true, size: 10, name: TNR };
    c.alignment = { horizontal: 'center' };
    c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFDCEEF1' } };
  });
  r++;

  const colHeadRow = r;
  const HEAD = ['TANGGAL', 'LEVEL (m)', 'ELEV. 3', 'ELEV. 5', 'ELEV. 7', 'NTU', 'PH'];
  HEAD.forEach((h, i) => { ws.getCell(`${colLetter(1 + i)}${colHeadRow}`).value = h; });
  HEAD.forEach((h, i) => { ws.getCell(`${colLetter(9 + i)}${colHeadRow}`).value = h; });
  for (let c = 1; c <= KUALITAS_LAST_COL; c++) {
    if (c === KUALITAS_GAP_COL) continue;
    const cell = ws.getRow(colHeadRow).getCell(c);
    cell.font = { bold: true, size: 9, name: TNR };
    cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
    cell.border = GRID;
  }
  r = colHeadRow + 1;
  ...
  const acc = {
    manggar: { level: [], ntu: [], ph: [] },
    teritip: { level: [], ntu: [], ph: [] }
  };

  function writeLocDay(colBase, locKey, ds, d) {
    const tCol = colLetter(colBase), lCol = colLetter(colBase + 1);
    const e3Col = colLetter(colBase + 2), e5Col = colLetter(colBase + 3), e7Col = colLetter(colBase + 4);
    const nCol = colLetter(colBase + 5), pCol = colLetter(colBase + 6);

    ws.getCell(`${tCol}${r}`).value = d;

    const lvl = dailyByLocation[locKey].level[ds];
    if (lvl !== undefined && lvl !== null) { ws.getCell(`${lCol}${r}`).value = lvl; acc[locKey].level.push(lvl); }
    ws.getCell(`${lCol}${r}`).numFmt = '0.00';

    const dayRow = elevasiDailyMap[ds] || { manggar: { 3: false, 5: false, 7: false }, teritip: { 3: false, 5: false, 7: false } };
    const ev = dayRow[locKey];
    [[e3Col, 3], [e5Col, 5], [e7Col, 7]].forEach(pair => {
      const col = pair[0], e = pair[1];
      const on = !!ev[e];
      const cell = ws.getCell(`${col}${r}`);
      cell.value = on ? 'ON' : 'OFF';
      cell.font = on
        ? { size: 9, name: TNR, bold: true, color: { argb: 'FF3E9B6F' } }
        : { size: 9, name: TNR, color: { argb: 'FF9AA9AD' } };
    });

    const ntu = dailyByLocation[locKey].ntu[ds];
    if (ntu !== undefined && ntu !== null) { ws.getCell(`${nCol}${r}`).value = ntu; acc[locKey].ntu.push(ntu); }
    ws.getCell(`${nCol}${r}`).numFmt = '0.0';

    const ph = dailyByLocation[locKey].ph[ds];
    if (ph !== undefined && ph !== null) { ws.getCell(`${pCol}${r}`).value = ph; acc[locKey].ph.push(ph); }
    ws.getCell(`${pCol}${r}`).numFmt = '0.00';
  }

  for (let d = 1; d <= days; d++) {
    const ds = `${year}-${pad2(monthIndex + 1)}-${pad2(d)}`;
    writeLocDay(1, 'manggar', ds, d);
    writeLocDay(9, 'teritip', ds, d);
    r++;
  }
  ...
  for (let rr = firstDataRow; rr <= lastDataRow; rr++) {
    for (let c = 1; c <= KUALITAS_LAST_COL; c++) {
      if (c === KUALITAS_GAP_COL) continue;
      const cell = ws.getRow(rr).getCell(c);
      if (!cell.font) cell.font = { size: 9, name: TNR };
      cell.alignment = cell.alignment || { horizontal: 'center' };
      cell.border = { left: THIN, right: THIN, top: HAIR, bottom: rr === lastDataRow ? THIN : HAIR };
    }
  }
  ...
  function summaryRow(label, calcFn, funcName) {
    ws.getCell(`A${r}`).value = label;
    ws.getCell(`A${r}`).font = { bold: true, size: 9, name: TNR };
    function put(col, arr, numFmt) {
      const cell = ws.getCell(`${col}${r}`);
      if (arr.length) {
        cell.value = {
          formula: `IF(COUNT(${col}${firstDataRow}:${col}${lastDataRow})=0,"",${funcName}(${col}${firstDataRow}:${col}${lastDataRow}))`,
          result: calcFn(arr)
        };
      }
      cell.numFmt = numFmt;
    ...
    put('B', acc.manggar.level, '0.00');
    put('F', acc.manggar.ntu, '0.0');
    put('G', acc.manggar.ph, '0.00');
    put('J', acc.teritip.level, '0.00');
    put('N', acc.teritip.ntu, '0.0');
    put('O', acc.teritip.ph, '0.00');
    for (let c = 1; c <= KUALITAS_LAST_COL; c++) {
      if (c === KUALITAS_GAP_COL) continue;
      ws.getRow(r).getCell(c).border = GRID;
    }
    r++;
  }
  summaryRow('RATA-RATA', avg, 'AVERAGE');
  summaryRow('TERTINGGI', max, 'MAX');
  summaryRow('TERENDAH', min, 'MIN');
  r++;

  return buildKualitasSignBlockDaily(ws, r, meta);
}

async function buildKpiKualitasExcelWorkbook(data) {
  const wb = new ExcelJS.Workbook();
  wb.calcProperties.fullCalcOnLoad = true;
  const ws = wb.addWorksheet('18.4 KUALITAS AIR BAKU', { pageSetup: { orientation: 'landscape', fitToPage: true } });
  KUALITAS_COL_WIDTHS.forEach((w, i) => { ws.getColumn(i + 1).width = w; });

  const elevasiDailyMap = elevasiDailyToMap(data.elevasiDaily);

  let r = 1;
  for (let mi = 0; mi < 12; mi++) {
    r = buildKualitasMonthBlockExcel(ws, r, data.year, mi, data.dailyByLocation, elevasiDailyMap, data.meta);
    r++;
  }

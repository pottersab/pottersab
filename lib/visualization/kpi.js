...
const KUALITAS_LOCATIONS = [
  { key: 'manggar', label: 'Waduk Manggar', levelDs: 'manggar_level', ntuDs: 'manggar_ntu', phDs: 'manggar_ph',
    title: '18.4a LAPORAN KWALITAS AIR Baku & Lingkungan WADUK MANGGAR/IPAM km 12' },
...
function buildLocationMonth(loc, levelMap, ntuMap, phMap, elevByDate, year, monthIndex) {
  ...
  return {
    key: loc.key, label: loc.label, title: loc.title,
    rows,
...
// sama persis seperti KPI lain di atas. Meniru PERSIS lampiran contoh
// (18.4 Laporan Kualitas Air Baku.xlsx): SATU bulan per file, header
// "PEMERINTAH KOTA BALIKPAPAN / PERUSAHAAN DAERAH AIR MINUM", font Calibri,
// dua lokasi BERDAMPINGAN (Manggar kiri, Teritip kanan), baris HARIAN
// 1..jumlah-hari dengan kolom TANGGAL/LEVEL/ELEVASI 3,5,7/NTU/PH + kolom
// acuan baku mutu "NO.82 TH 2001", ditutup Rata-rata/Tertinggi/Terendah dan
// tanda tangan. ---
const KUAL_FONT = 'Calibri';
// Per lokasi 8 kolom: TANGGAL, LEVEL, ELEV.3, ELEV.5, ELEV.7, NTU, PH,
// NO.82 TH 2001. Kolom 9 (I) = pemisah. Lokasi kedua kolom 10-17 (J-Q).
const KUAL_COL_WIDTHS = [9.5, 8, 7, 7, 7, 8, 8, 12, 3, 9.5, 8, 7, 7, 7, 8, 8, 12];
const KUAL_LAST_COL = 17;   // 'Q'
const KUAL_GAP_COL = 9;     // 'I'
const KUAL_MANGGAR_BASE = 1;
const KUAL_TERITIP_BASE = 10;

function kualCellFont(opts) { return Object.assign({ size: 10, name: KUAL_FONT }, opts || {}); }

function buildKualitasSignBlockMonth(ws, r, meta) {
  const ketHeaderRow = r;
  ...
  // Tanda tangan di blok kanan (kolom Teritip), sejajar dua kolom.
  ws.mergeCells(`N${lastKetRow}:Q${lastKetRow}`);
  ws.getCell(`N${lastKetRow}`).value = meta.signPlaceDate || '';
  ws.getCell(`N${lastKetRow}`).font = kualCellFont();
  ws.getCell(`N${lastKetRow}`).alignment = { horizontal: 'center' };
  ...
  ws.getCell(`A${labelRow}`).alignment = { horizontal: 'center' };
  ws.mergeCells(`N${labelRow}:Q${labelRow}`);
  ws.getCell(`N${labelRow}`).value = KUALITAS_LABEL_RIGHT;
  ws.getCell(`N${labelRow}`).font = kualCellFont();
  ...
  ws.getCell(`A${roleRow}`).alignment = { horizontal: 'center' };
  ws.mergeCells(`N${roleRow}:Q${roleRow + 1}`);
  ws.getCell(`N${roleRow}`).value = meta.roleRight || '';
  ws.getCell(`N${roleRow}`).font = kualCellFont();
  ws.getCell(`N${roleRow}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ...
  ws.getCell(`A${nameRow}`).alignment = { horizontal: 'center' };
  ws.mergeCells(`N${nameRow}:Q${nameRow}`);
  ws.getCell(`N${nameRow}`).value = meta.nameRight || '';
  ws.getCell(`N${nameRow}`).font = kualCellFont({ bold: true });
  ws.getCell(`N${nameRow}`).alignment = { horizontal: 'center' };

  return nameRow + 2;
}

function buildKualitasLocationTableExcel(ws, base, colHeaderRow, dataTopRow, loc) {
  // colHeaderRow: baris pertama header kolom (2 baris). dataTopRow: baris
  // pertama data harian. Judul lokasi & baris BULAN digambar di pemanggil.
  const c = i => colLetter(base + i); // 0..7 -> TANGGAL..NO.82

  // Header kolom: 2 baris (ELEVASI span 3, angka 3/5/7 di bawahnya)
  const h1 = colHeaderRow, h2 = colHeaderRow + 1;
  ...
  ws.mergeCells(`${c(5)}${h1}:${c(5)}${h2}`); ws.getCell(`${c(5)}${h1}`).value = 'NTU';
  ws.mergeCells(`${c(6)}${h1}:${c(6)}${h2}`); ws.getCell(`${c(6)}${h1}`).value = 'PH';
  ws.mergeCells(`${c(7)}${h1}:${c(7)}${h2}`); ws.getCell(`${c(7)}${h1}`).value = 'NO.82 TH 2001';
  for (let rr = h1; rr <= h2; rr++) {
    for (let i = 0; i <= 7; i++) {
      const cell = ws.getCell(`${c(i)}${rr}`);
      cell.font = kualCellFont({ bold: true });
      cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
      cell.border = GRID;
    }
  }
  ...
  loc.rows.forEach(row => {
    ...
    [[2, row.e3], [3, row.e5], [4, row.e7]].forEach(pair => {
      const cell = ws.getCell(`${c(pair[0])}${r}`);
      cell.value = pair[1] ? 'ON' : 'OFF';
      cell.font = kualCellFont(pair[1] ? { bold: true } : {});
    });
  ...
  // Ringkasan
  const summaries = [
    ['Rata rata', loc.summary, 'avg', 'AVERAGE'],
    ['Tertinggi', loc.summary, 'max', 'MAX'],
    ['Terendah', loc.summary, 'min', 'MIN']
  ];
  summaries.forEach(sdef => {
    const label = sdef[0], key = sdef[2], fn = sdef[3];
    ws.getCell(`${c(0)}${r}`).value = label;
    ws.getCell(`${c(0)}${r}`).font = kualCellFont({ bold: true });
    ...
    for (let i = 0; i <= 7; i++) ws.getCell(`${c(i)}${r}`).border = GRID;
    r++;
  });

  return r; // baris setelah ringkasan
}
...
async function buildKpiKualitasExcelWorkbook(data) {
  ...
  const manggar = data.locations.find(l => l.key === 'manggar');
  const teritip = data.locations.find(l => l.key === 'teritip');

  // Kop pemerintah (dua kolom, kiri & kanan) -- persis contoh.
  ws.getCell('A1').value = 'PEMERINTAH KOTA BALIKPAPAN';
  ws.getCell('J1').value = 'PEMERINTAH KOTA BALIKPAPAN';
  ws.getCell('A2').value = 'PERUSAHAAN DAERAH AIR MINUM';
  ws.getCell('J2').value = 'PERUSAHAAN DAERAH AIR MINUM';
  ['A1', 'J1', 'A2', 'J2'].forEach(ref => { ws.getCell(ref).font = kualCellFont({ bold: true, size: 11 }); });

  // Baris 3: judul laporan per lokasi (18.4a Manggar kiri, 18.4b Teritip kanan)
  const drawTitle = (startCol, endCol, title) => {
    ws.mergeCells(`${startCol}3:${endCol}3`);
    const cell = ws.getCell(`${startCol}3`);
    cell.value = title;
    cell.font = kualCellFont({ bold: true, size: 11 });
    cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  };
  drawTitle('A', 'H', manggar.title);
  drawTitle('J', 'Q', teritip.title);

  // Baris 4: BULAN (per blok, mengikuti contoh yang menaruhnya di bawah judul)
  const drawBulan = (startCol, endCol) => {
    ws.mergeCells(`${startCol}4:${endCol}4`);
    const cell = ws.getCell(`${startCol}4`);
    cell.value = `BULAN : ${data.monthTitle}`;
    cell.font = kualCellFont({ bold: true, size: 11 });
  ...
  // Header kolom baris 5-6, data mulai baris 7.
  const colHeaderRow = 5, dataTopRow = 7;
  const endLeft = buildKualitasLocationTableExcel(ws, KUAL_MANGGAR_BASE, colHeaderRow, dataTopRow, manggar);
  const endRight = buildKualitasLocationTableExcel(ws, KUAL_TERITIP_BASE, colHeaderRow, dataTopRow, teritip);

  const afterTables = Math.max(endLeft, endRight) + 1;
  buildKualitasSignBlockMonth(ws, afterTables, data.meta);

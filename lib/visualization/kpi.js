...
function buildPengambilanExcelBlock(ws, rowOffset, monthNames, monthBase, judulBulan, data) {
  ...
  // Bulan berjalan zona WITA -- dipakai menyembunyikan Anggaran bulan-bulan
  // sebelum bulan berjalan (revisi user).
  const curYm = currentYmWITA();
  const curYear = Number(curYm.slice(0, 4));
  const curMi = Number(curYm.slice(5, 7)) - 1;
  const reportYear = Number(data.year);
  const isPastMonth = mi => reportYear < curYear || (reportYear === curYear && mi < curMi);
  ...
  data.groups.forEach((g, gi) => {
    ...
    monthNames.forEach((mn, i) => {
      ...
      // Anggaran bulan-bulan sebelum bulan berjalan disembunyikan (dahulu).
      if (anggVal !== null && anggVal !== undefined && !isPastMonth(monthBase + i)) anggCell.value = anggVal;
      const realCell = ws.getCell(`${rc}${r}`);
...
function buildKpi9_5ExcelWorkbook(data) {
  ...
  // Jabaran (Supervisor / Manajer Produksi) di-rata-ATAS -- revisi user (dulu
  // di tengah vertikal pada blok merge 4 baris).
  ...
  // Kode dokumen (kotak tipis, kiri bawah)
  setv(1, codeRow, meta.footerCode || '', ar10b);
  bdT(1, codeRow, 'tlb'); bdT(2, codeRow, 'tb'); bdT(3, codeRow, 'trb');

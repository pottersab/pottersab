...
// Jumlah sumur aktif (anggaran) per instalasi -- satu nilai per instalasi.
async function fetchLevelWellCounts() {
  const { rows } = await pool.query(
    `SELECT installation, COUNT(*)::int AS n FROM sumur_wells WHERE category = 'level' GROUP BY installation`
  );
  const map = {};
  rows.forEach(r => { map[r.installation] = r.n; });
  return map;
}
...
function countLevelRealByMonth(rows, year) {
  const counts = new Array(12).fill(0);
  const seen = Array.from({ length: 12 }, () => new Set());
  rows.forEach(r => {
    if (r.bulan.slice(0, 4) !== String(year)) return;
    const mi = Number(r.bulan.slice(5, 7)) - 1;
    if (!seen[mi].has(r.well_name)) { seen[mi].add(r.well_name); counts[mi]++; }
  });
  return counts;
}
...
async function getKpiLevelSumurData(access, tahunQuery) {
  ...
  const [wellCounts, ...readings] = await Promise.all([
    fetchLevelWellCounts(),
  ...
  const groups = KPI_18_1A_INSTALLATIONS.map((inst, i) => ({
    installation: inst.installation, label: inst.label,
    angg: wellCounts[inst.installation] || 0,
    real: countLevelRealByMonth(readings[i], year)
  }));
...
function buildLevelSumurExcelBlock(ws, rowOffset, monthNames, monthBase, judulBulan, data) {
  ...
  data.groups.forEach((g, gi) => {
    ...
    monthNames.forEach((mn, i) => {
      ...
      const anggCell = ws.getCell(`${ac}${r}`);
      const realVal = g.real[monthBase + i];
      if (g.angg !== null && g.angg !== undefined) anggCell.value = g.angg;
      const realCell = ws.getCell(`${rc}${r}`);
      if (realVal !== null && realVal !== undefined) realCell.value = realVal;
      ws.getCell(`${pmc}${r}`).value = { formula: `IF(OR(${rc}${r}="",${ac}${r}=""),"",${rc}${r}-${ac}${r})` };
      ws.getCell(`${pctc}${r}`).value = { formula: `IF(OR(${rc}${r}="",${ac}${r}=""),"",${rc}${r}/${ac}${r}*100)` };
  ...
  monthNames.forEach((mn, i) => {
    ...
    data.groups.forEach(g => {
      ...
      if (g.angg) sumAngg += g.angg;
      if (rv) { sumReal += rv; any = true; }
    });
    ...
    realCell.value = { formula: `SUM(${rc}${firstDataRow}:${rc}${lastDataRow})`, result: any ? sumReal : '' };
    realCell.numFmt = '#,##0';

...
const ACTIVITY_PLAN_GROUPS = [
  {
    ...
    items: [
      ...
      { key: '18.3a', label: '18.3a Laporan pengambilan air Baku & Lingkungan', kind: 'manual', target: '0.89', timing: 'Monthly' },
...
const ACTIVITY_PLAN_MONTHS = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun'];

function activityPctRound(v) { return Math.round(v * 100) / 100; }

// Rata-rata persentase per bulan (Jan-Jun) dari laporan KPI terkait.
async function activityAutoPct(source, tahun) {
  const pct = new Array(6).fill(null);
  if (source === 'level') {
    const data = await getKpiLevelSumurData({ granted: true, kind: 'admin' }, tahun);
    for (let mi = 0; mi < 6; mi++) {
      let sumA = 0, sumR = 0;
      data.groups.forEach(g => { if (g.angg) sumA += g.angg; const r = g.real[mi]; if (r !== null && r !== undefined) sumR += r; });
  ...
  if (source === 'debit') {
    ...
    for (let mi = 0; mi < 6; mi++) {
      const vals = [];
      data.groups.forEach(g => g.wells.forEach(w => { const a = w.awal, r = w.real[mi]; if (a && a > 0 && r !== null && r !== undefined) vals.push(r / a * 100); }));
  ...
  if (g) for (let mi = 0; mi < 6; mi++) { const a = g.angg[mi], r = g.real[mi]; if (a && a > 0 && r !== null && r !== undefined) pct[mi] = activityPctRound(r / a * 100); }
  return pct;
}
...
// Rata-rata values per bulan dari daftar baris (lewati sel kosong).
function activityAverage(rows) {
  const avg = new Array(6).fill(null);
  for (let mi = 0; mi < 6; mi++) {
    const vals = rows.map(r => r.values[mi]).filter(v => v !== null && v !== undefined);
...
function activityStatus(values, target, def) {
  ...
  if (target !== '' && target !== '-' && !isNaN(t)) {
    for (let mi = 5; mi >= 0; mi--) if (values[mi] !== null && values[mi] !== undefined) return values[mi] >= t ? 'TCP' : 'TDTCP';
  }
  return def || '';
}

async function getKpiActivityPlanData(access, tahunQuery) {
  const currentYear = String(new Date().getFullYear());
  let tahun = (typeof tahunQuery === 'string' && /^\d{4}$/.test(tahunQuery)) ? tahunQuery : currentYear;
  ...
  const auto = {};
  if (granted) {
    auto.level = await activityAutoPct('level', tahun);
    auto.debit = await activityAutoPct('debit', tahun);
    auto.pengambilan_ap = await activityAutoPct('pengambilan_ap', tahun);
    auto.pengambilan_atd = await activityAutoPct('pengambilan_atd', tahun);
  }
  ...
  const groups = ACTIVITY_PLAN_GROUPS.map(g => {
    ...
    const collect = (item, level) => {
      ...
      if (item.kind === 'auto') {
        ...
        for (let mi = 0; mi < 6; mi++) {
          if (mv[mi] !== undefined && mv[mi] !== null && mv[mi] !== '') values[mi] = Number(mv[mi]);
  ...
  return { locked: !granted, availableYears: [], tahun, periode: 'JANUARI - DESEMBER ' + tahun, groups };
}
...
// Meniru Activity Plan SAB.xlsx: kop, judul, PERIODE/BAGIAN/SUB BAGIAN, header
// kolom (Key Result Area, Action plan & Strategy, Target, Check Timing,
// Progres Jan-Jun, Status, Trend, Problem, Corrective Action, PIC, Due Date,
// Progres), baris grup + item + sub-item, dan blok tanda tangan.
const ACTIVITY_PLAN_COL_WIDTHS = [4.5, 6.7, 3.5, 38.6, 77.6, 16, 16.6, 10.6, 10.6, 10.6, 10.6, 11.3, 10.6, 11.3, 10.3, 42.4, 18.3, 18.4, 20.4, 27.3, 20.6];
...
function buildKpiActivityPlanExcelWorkbook(data) {
  ...
  ws.pageSetup.margins = { left: 0.25, right: 0.25, top: 0.75, bottom: 0.75, header: 0.3, footer: 0.3 };
  ACTIVITY_PLAN_COL_WIDTHS.forEach((w, i) => { ws.getColumn(i + 1).width = w; });

  // Kop (Arial 12 tebal)
  ws.getCell('C1').value = 'PERUSAHAAN UMUM DAERAH TIRTA MANUNTUNG';
  ws.getCell('C1').font = { bold: true, size: 12, name: 'Arial' };
  ws.getCell('C2').value = 'KOTA BALIKPAPAN';
  ws.getCell('C2').font = { bold: true, size: 12, name: 'Arial' };
  ws.getRow(1).height = 32.25;
  ws.getRow(2).height = 32.25;

  // Judul
  ws.mergeCells('B4:V4');
  ws.getCell('B4').value = 'ACTIVITY PLAN PERUMDA TIRTA MANUNTUNG BALIKPAPAN';
  ws.getCell('B4').font = { bold: true, size: 14, name: 'Calibri' };
  ws.getCell('B4').alignment = { horizontal: 'center' };
  ws.getRow(4).height = 25.5;

  // PERIODE / BAGIAN / SUB BAGIAN
  ws.mergeCells('B5:Q5');
  ws.getCell('B5').value = 'PERIODE: ' + (data.periode || '');
  ws.getCell('B5').font = { bold: true, size: 12, name: 'Calibri' };
  ws.getRow(5).height = 15.75;
  ws.mergeCells('B6:Q6');
  ws.getCell('B6').value = 'BAGIAN : PRODUKSI';
  ws.getCell('B6').font = { bold: true, size: 12, name: 'Calibri' };
  ws.getRow(6).height = 15.75;
  ws.mergeCells('B7:Q8');
  ws.getCell('B7').value = 'SUB BAGIAN SUMBER AIR DAN LINGKUNGAN';
  ws.getCell('B7').font = { bold: true, size: 12, name: 'Calibri' };

  // Header kolom (2 baris)
  const H1 = 10, H2 = 11;
  const headerDefs = [
    ['B', H1, H2, 'Key Result Area'], ['E', H1, H1, 'Action plan & Strategy'], ['F', H1, H1, 'Target'],
    ['G', H1, H1, 'Check Timing'], ['N', H1, H1, 'Status'], ['O', H1, H1, 'Trend'], ['P', H1, H1, 'Problem'],
    ['Q', H1, H1, 'Corective Action'], ['S', H1, H1, 'PIC'], ['T', H1, H1, 'Due Date'], ['U', H1, H1, 'Progres']
  ];
  headerDefs.forEach(([c, r1, r2, label]) => {
    ws.mergeCells(`${c}${r1}:${c}${r2}`);
    ws.getCell(`${c}${r1}`).value = label;
    ws.getCell(`${c}${r1}`).font = { bold: true, size: 10, name: 'Calibri' };
    ws.getCell(`${c}${r1}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  });
  // Progres Jan-Jun (H-M) baris 1
  ACTIVITY_PLAN_MONTHS.forEach((mn, i) => {
    const c = colLetter(8 + i);
    ws.getCell(`${c}${H1}`).value = 'Progres ' + (data.tahun || '');
    ws.getCell(`${c}${H1}`).font = { bold: true, size: 10, name: 'Calibri' };
    ws.getCell(`${c}${H1}`).alignment = { horizontal: 'center', vertical: 'middle' };
    ws.getCell(`${c}${H2}`).value = mn;
    ws.getCell(`${c}${H2}`).font = { bold: true, size: 10, name: 'Calibri' };
    ws.getCell(`${c}${H2}`).alignment = { horizontal: 'center', vertical: 'middle' };
  });
  for (let rr = H1; rr <= H2; rr++) for (let c = 2; c <= 21; c++) ws.getRow(rr).getCell(c).border = GRID;

  // Data: grup + item + sub-item
  let r = H2 + 1;
  data.groups.forEach(g => {
    // baris grup
    writeActivityRow(ws, r, g.group, 0, true);
    r++;
    g.rows.forEach(row => { writeActivityRow(ws, r, row, row.level, false); r++; });
  });

  return wb.xlsx.writeBuffer();
}

function writeActivityRow(ws, r, row, level, isGroup) {
  const indent = level === 0 ? 'B' : (level === 1 ? 'C' : 'D');
  // NO & label
  ws.getCell(`B${r}`).value = row.isGroup ? row.key : '';
  ws.getCell(`${indent}${r}`).value = row.label;
  ws.getCell(`${indent}${r}`).font = { bold: isGroup, size: 11, name: 'Calibri' };
  ws.getCell(`${indent}${r}`).alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
  // Target & Check Timing
  ws.getCell(`F${r}`).value = row.target;
  ws.getCell(`G${r}`).value = row.timing;
  // Progres Jan-Jun
  ACTIVITY_PLAN_MONTHS.forEach((_, i) => {
    const c = colLetter(8 + i);
    const cell = ws.getCell(`${c}${r}`);
    if (row.values[i] !== null && row.values[i] !== undefined) cell.value = row.values[i];
    cell.font = { size: 11, name: 'Calibri' };
    cell.alignment = { horizontal: 'center', vertical: 'middle' };
  });
  ws.getCell(`N${r}`).value = row.status;
  ws.getCell(`P${r}`).value = row.problem;
  ws.getCell(`Q${r}`).value = row.corrective;
  ws.getCell(`S${r}`).value = row.pic;
  ws.getCell(`T${r}`).value = row.due_date;
  ws.getCell(`U${r}`).value = row.progres;
...
module.exports = {
  ...
  getKpiActivityPlanData, saveKpiActivityPlanRow, buildKpiActivityPlanExcelWorkbook
};

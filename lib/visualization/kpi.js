...
function buildKpi9_5ExcelWorkbook(data) {
  ...
  for (let r = 11; r <= 12; r++) {
    for (let c = 1; c <= 8; c++) {
      const b = CELL(c, r).border || {};
      b.top = THIN; b.left = THIN; b.right = THIN; b.bottom = THIN;
      CELL(c, r).border = b;
    }
  }
  // Baris 13: penutup header (garis atas saja).
  for (let c = 1; c <= 8; c++) bdT(c, 13, 't');
  ...
  pelLines.forEach((line, i) => {
    const pr = pelRows[i];
    const cell = setv(7, pr, line, ar10b);

...
// Warna isi sel penanda kegiatan berjalan hari itu (fill, tanpa teks centang)
// -- teal muda, senada --primary-pale di style.css.
const JK_FILL = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFDCEEF1' } };
  ...
  data.rows.forEach((row, ri) => {
    ...
    if (row.merged) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (row.cells[i]) cell.fill = JK_FILL;  // penanda = warna sel, tanpa teks

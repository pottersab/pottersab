...
// Kelompok item persis file 18.6: `def` menunjuk indeks KPI_19_2_DEFS (0-3).
// Tiap item punya 2 baris: Rencana (label `renc`) & Realisasi (label `real`).
const KPI_18_6_GROUPS = [
  { no: 1, items: [{ renc: 'Jalur Pipa Transmisi', real: 'Km12 - km8', def: 0 }] },
  { no: 2, items: [{ renc: 'Jalur Pipa Transmisi', real: 'Km 8 - Kampung Damai', def: 1 }] },
  { no: 3, items: [
    { renc: 'Jalur Pipa Sumur dan', real: 'Jalur Pipa Sumur dan', def: 2 },
    { renc: 'Perawatan Sumur-sumur', real: 'Perawatan Sumur-sumur', def: 3 }
  ] }
];
...
async function getKpi18_6Data(access, tahunQuery) {
  ...
  const buildItemRows = (it, baDates) => {
    const def = KPI_19_2_DEFS[it.def];
    ...
    return { renc: it.renc, real: it.real, rencDates, realDates };
  };

  if (!access.granted) {
    return {
      locked: true, availableYears: [], tahun,
      groups: KPI_18_6_GROUPS.map(g => ({ no: g.no, items: g.items.map(it => buildItemRows(it, null)) })),
  ...
  const groups = KPI_18_6_GROUPS.map(g => ({
    no: g.no,
    items: g.items.map(it => buildItemRows(it, baByDef[it.def] || []))
  }));
...
function buildKpi18_6Block(ws, rowOffset, monthIdx, judulBulan, data, meta, tahun) {
  ...
  ws.getCell(`A${R(4)}`).alignment = { horizontal: 'center' };
  // Tinggi baris kop/judul/bulan 15.6 -- persis file contoh.
  ws.getRow(R(1)).height = 15.6;
  ws.getRow(R(2)).height = 15.6;
  ws.getRow(R(3)).height = 15.6;
  ws.getRow(R(4)).height = 15.6;
  ...
  // Data: tiap sub-baris Rencana/Realisasi = 3 BARIS di-merge vertikal (persis
  // file contoh), jadi tinggi tiap blok sama dengan panduan. NO di-merge
  // sepanjang kelompok; URAIAN (B:D), kolom Renc/Real (E), dan tiap bulan
  // (F-K) masing-masing di-merge 3 baris.
  let r = R(7);
  data.groups.forEach(g => {
    const firstRow = r;
    const totalRows = g.items.length * 2 * 3;
    ...
    g.items.forEach(it => {
      // Kalau label Rencana & Realisasi SAMA (mis. NO 3), URAIAN di-merge 6
      // baris sekaligus mencakup Renc+Real -- persis file contoh. Kalau beda
      // (NO 1/2), masing-masing sub-baris di-merge 3 baris.
      const sameLabel = it.renc === it.real;
      const subRows = [[it.renc, 'Renc', it.rencDates], [it.real, 'Real', it.realDates]];
      subRows.forEach(([label, kind, dates], si) => {
        if (!sameLabel || si === 0) {
          const end = sameLabel ? r + 5 : r + 2;
          ws.mergeCells(`B${r}:D${end}`);

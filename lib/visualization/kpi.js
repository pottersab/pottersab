...
// Border tengah (antar-baris & antar-kolom di dalam tabel) putus-putus,
// pinggir luar solid tipis -- permintaan user.
...
// Border sel data: perimeter tabel solid (THIN), garis tengah putus-putus.
function kualDataBorder(rr, firstRow, lastRow, i) {
  return {
    top: rr === firstRow ? THIN : KUAL_DASH,
    bottom: rr === lastRow ? THIN : KUAL_DASH,
    left: i === 0 ? THIN : KUAL_DASH,
    right: i === 7 ? THIN : KUAL_DASH
  };
}
...
function buildKualitasSheet(wb, loc, monthTitle, meta) {
  ...
  for (let rr = firstDataRow; rr <= lastDataRow; rr++) {
    for (let i = 0; i <= 7; i++) {
      ...
      cell.border = kualDataBorder(rr, firstDataRow, lastDataRow, i);
    }
  }

...
function buildApatdSubTable(ws, r, judul, judulBulan, group, monthBase) {
  ...
  for (let m = 0; m < 6; m++) {
    const col = colLetter(3 + m);
    const cell = ws.getCell(`${col}${r}`);
    cell.value = { formula: `IF(COUNT(${col}${firstRow}:${col}${lastRow})=0,"",SUM(${col}${firstRow}:${col}${lastRow}))` };
    cell.numFmt = '#,##0';
  }
  for (let c = 1; c <= 8; c++) ws.getRow(r).getCell(c).font = { bold: true, size: 11, name: TNR };

  for (let rr = firstRow - 2; rr <= r; rr++) {
    for (let c = 1; c <= 8; c++) {
      const cell = ws.getRow(rr).getCell(c);

...
//
// opts { start, end } (string 'YYYY-MM-DD') opsional: kalau dikirim, baris
// yang difilter di SQL (tanggal >= start AND tanggal <= end). Dipakai pemanggil
// yang cuma butuh satu bulan/tahun laporan (KPI 18.4/9.2/9.3) supaya tidak
// menarik ribuan baris harian cuma untuk dibuang lagi di memori. Tanpa opts
// perilakunya tetap sama seperti sebelumnya (semua baris).
async function fetchWideSingleRows(source, opts) {
  const dateKey = source.dateGranularity === 'day' ? 'Tanggal' : 'Bulan';
  const dateFormat = source.dateGranularity === 'day' ? 'YYYY-MM-DD' : 'YYYY-MM';
  const where = (opts && opts.start && opts.end)
    ? ` WHERE ${source.dateCol} >= $1 AND ${source.dateCol} <= $2`
    : '';
  const params = (opts && opts.start && opts.end) ? [opts.start, opts.end] : [];
  const { rows: dbRows } = await pool.query(
    `SELECT to_char(${source.dateCol}, '${dateFormat}') as d, ${source.col} FROM ${source.table}${where} ORDER BY ${source.dateCol}`,
    params
  );

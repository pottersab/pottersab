...
const MARGIN = 40;
const ROW_HEIGHT = 16;
const SUBTITLE_TINGGI = 18;
// Jarak antar tabel yang berbagi satu halaman (lihat section.sambung), dengan
// garis pemisah tipis di tengahnya. Dibuat selega ini karena halaman lanskap
// berisi dua tabel 6 baris masih menyisakan banyak ruang kosong di bawah --
// jarak yang sempit cuma membuat dua tahun terbaca seperti satu tabel panjang,
// tanpa menghemat apa pun.
const JEDA_SECTION = 54;
const GARIS_PEMISAH = rgb(199 / 255, 218 / 255, 221 / 255);
...
const PENANDA = [
  ...
  'Data diinput dan direkap oleh Staff',
...
// subjudul + tabel sendiri). Secara bawaan tiap section mulai di halaman baru;
// section.sambung = true membuatnya menumpang di halaman berjalan selama masih
// muat. Header tabel berulang tiap kali baris meluap ke halaman baru dalam
// section yang sama.
...
async function buildTablePdf({ landscape, sections }) {
  ...
  for (const section of sections) {
    ...
    // section.sambung = tabel ini boleh menumpang di halaman yang sedang
    // berjalan, bukan memulai halaman baru. Dipakai rekapitulasi semua tahun
    // supaya dua tahun muat dalam satu halaman, bukan satu tahun satu halaman.
    // Judul dokumen tidak diulang -- yang sudah tercetak di atas berlaku juga
    // untuk tabel ini; subjudulnya yang membedakan tahun.
    const tinggiSection = (section.subtitle ? SUBTITLE_TINGGI : 6)
      + ROW_HEIGHT + section.rows.length * ROW_HEIGHT;
    const muatDiHalamanIni = section.sambung && page
      && (y - JEDA_SECTION - tinggiSection) >= MARGIN;

    if (muatDiHalamanIni) {
      // Garis pemisah persis di tengah jeda, jadi jarak ke tabel di atas dan
      // ke subjudul di bawah sama besar.
      y -= JEDA_SECTION / 2;
      page.drawLine({
        start: { x: MARGIN, y }, end: { x: MARGIN + contentWidth, y },
        thickness: 0.75, color: GARIS_PEMISAH
      });
      y -= JEDA_SECTION / 2;
    } else {
      tambahHalaman();

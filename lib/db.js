...
async function ensureKpiTables() {
  ...
  await pool.query(`
    ...
    -- rekap KPI Sub Bagian Sumber Air & Lingkungan (mengikuti Activity Plan
    -- SAB.xlsx). Nilai Progres per bulan: untuk 18.1a/18.2/18.3b otomatis
    -- dihitung dari laporan KPI terkait, untuk lainnya manual (default 100),
    -- grup 18/19 rata-rata dari itemnya. Yang disimpan di sini cuma kolom
    -- manual per item: target, check_timing, status (bisa di-override; default
    -- TCP/TDTCP dihitung dari % vs target), trend, problem, corrective, pic,
    -- due_date, progres, dan values (JSON per indeks bulan 0-5 untuk item
    -- manual).

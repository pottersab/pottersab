...
async function ensureKpiTables() {
  ...
  await pool.query(`
    ...
    -- 9.2 Laporan Kualitas Air Baku (apps/kpi-sab/kualitas-9-2.html). Datanya
    -- SAMA dengan 18.4 (Level/NTU/PH + status pintu elevasi dari
    -- kualitas_elevasi_log, lihat kpi.js) -- yang disimpan di sini cuma
    -- penandatangan TIGA kolom (Diketahui / Mengetahui-Menyetujui / Pelaksana)
    -- dan kode dokumen footer (default "PTMBPP-IR-PRD.SAB/01-02"), global.
    CREATE TABLE IF NOT EXISTS kpi_9_2_meta (
      period_key TEXT PRIMARY KEY,
      role_diketahui TEXT,
      name_diketahui TEXT,
      role_mengetahui TEXT,
      name_mengetahui TEXT,
      name_pelaksana TEXT,
      footer_code TEXT,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );

    -- 9.3 Laporan Kondisi Air Waduk (apps/kpi-sab/kondisi-9-3.html). Datanya
    -- harian (Level & Curah Hujan dari manggar_level_curahhujan, Volume dari
    -- rumus level, Air yang disadap dari air_permukaan Kampung Damai + Batu
    -- Ampar + Km 12, Volume setelah diambil = Volume - disadap) -- lihat
    -- kpi.js. Yang disimpan di sini cuma penandatangan TIGA kolom (Diketahui /
    -- Mengetahui-Menyetujui / Pelaksana) dan kode dokumen footer (default
    -- "PTMBPP-IR-PRD.SAB/01-03"), global.
    CREATE TABLE IF NOT EXISTS kpi_9_3_meta (
      period_key TEXT PRIMARY KEY,
      role_diketahui TEXT,
      name_diketahui TEXT,
      role_mengetahui TEXT,
    ...
    -- 9.3 Laporan Kondisi Air Waduk: nilai harian "Air yang disadap" (m3) per
    -- bulan -- DIISI ADMIN manual (bukan otomatis dari AP), lewat form paste
    -- di halaman kondisi-9-3.html. Kolom "values" = array JSON, indeks 0 =
    -- tanggal 1, dst (null = kosong). Dipakai untuk menghitung "Volume setelah
    -- diambil" & baris ringkasan.
    CREATE TABLE IF NOT EXISTS kpi_9_3_disadap (
      bulan DATE PRIMARY KEY,
    ...
    -- Semua isian MANUAL oleh admin: daftar ketidaksesuaian per bulan disimpan
    -- di kpi_9_4_items (items = array JSON, tiap entri { ds: debit sebelumnya,
    -- di: debit hari ini, ket: keterangan }), penandatangan 2 kolom + kode
    -- dokumen "PTMBPP-IR-PRD.SAB/01-04" disimpan di kpi_9_4_meta (global).
    CREATE TABLE IF NOT EXISTS kpi_9_4_meta (
      period_key TEXT PRIMARY KEY,
      role_menyetujui TEXT,
      name_menyetujui TEXT,
      role_diketahui TEXT,
      name_diketahui TEXT,
      footer_code TEXT,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    CREATE TABLE IF NOT EXISTS kpi_9_4_items (
      bulan DATE PRIMARY KEY,
    ...
    -- Satu workbook 5 sheet (5 IPA, Hal 1-5). Kolom LEVEL (statis/dinamis) &
    -- REALITA (debit) bulan lalu & bulan ini diambil LANGSUNG dari tabel web
    -- sumur_level_readings / sumur_debit_readings (tidak disimpan di sini).
    -- Yang disimpan admin cuma:
    --   kpi_9_7_items -> per (bulan, installation): rows JSON (array { no, merk,
    --     type, namePlate, keterangan }) + catatan (teks bebas, satu baris per
    --     butir). NO diisi otomatis dari nomor sumur di data web.
    --   kpi_9_7_meta  -> global: penandatangan 3 kolom (Diketahui / Mengetahui-
    --     Menyetujui / Pelaksana), kode dokumen "PTMBPP-IR-PRD.SAB/01-07",
    --     total_halaman (default 5), dan halaman JSON (per-IPA: nomor Hal,
    --     default 1-5 sesuai urutan sheet).
    CREATE TABLE IF NOT EXISTS kpi_9_7_meta (
      period_key TEXT PRIMARY KEY,
      role_diketahui TEXT,
      name_diketahui TEXT,
      role_mengetahui TEXT,
    ...
    CREATE TABLE IF NOT EXISTS kpi_9_7_items (
      bulan DATE NOT NULL,
      installation TEXT NOT NULL,
      rows JSONB NOT NULL,
      catatan TEXT,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
      PRIMARY KEY (bulan, installation)
    );

    -- 9.5 Laporan Monitoring Pipa Transmisi (apps/kpi-sab/monitoring-9-5.html).
    -- SATU sheet "MONITORING PIPA 05" per bulan, 2 baris monitoring tetap:
    --   NO 1 = Selasa minggu ke-2, NO 2 = Selasa minggu ke-4 (kpi192Tanggal).
    -- Tanggal otomatis dari bulan; KEADAAN (AIR VALVE / VALVE WASH OUT /
    -- PERPIPAAN) & KETERANGAN diisi admin MANUAL per bulan (tidak ada data
    -- otomatis). Keterangan multi-baris (tiap baris = ~1 baris di Excel).
    --   kpi_9_5_items -> rows JSONB per bulan: [{ no, airValve, valveWashOut,
    --     perpipaan, keterangan }] (NO 1 & 2).
    --   kpi_9_5_meta  -> global: penandatangan 3 blok (Mengetahui [kiri],
    ...
    CREATE TABLE IF NOT EXISTS kpi_9_5_meta (
      period_key TEXT PRIMARY KEY,
      role_mengetahui TEXT,
      name_mengetahui TEXT,
      role_menyetujui TEXT,
    ...
    CREATE TABLE IF NOT EXISTS kpi_9_5_items (
      bulan DATE PRIMARY KEY,
      rows JSONB NOT NULL,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    ...
    -- SATU sheet "9" per bulan. SEMUA isian MANUAL oleh admin: daftar alat
    -- yang dikalibrasi (rows JSONB per bulan: [{ no, namaAlat, noSeri,
    -- kodeInventaris, terakhir, berikutnya, keterangan }]). Tidak ada data
    -- otomatis. Penandatangan 2 kolom (Mengetahui/Menyetujui kiri + Dibuat
    -- oleh kanan), kode dokumen "PTMBPP-IR-PRD.SAB/01-08", Lokasi & Halaman
    -- disimpan di kpi_9_8_meta (global).
    CREATE TABLE IF NOT EXISTS kpi_9_8_meta (
      period_key TEXT PRIMARY KEY,
      role_menyetujui TEXT,
      name_menyetujui TEXT,
      role_dibuat TEXT,
    ...
    CREATE TABLE IF NOT EXISTS kpi_9_8_items (
      bulan DATE PRIMARY KEY,
      rows JSONB NOT NULL,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );

...
async function ensureVizTables() {
  ...
  await pool.query(`
    ...
    -- Status ON/OFF pintu air elevasi 3/5/7 per hari, Waduk Manggar & Teritip
    -- (KPI 18.4 Laporan Kualitas Air Baku, apps/kpi-sab/kualitas.html). Beda
    -- dari tabel air baku lain di atas -- ini BUKAN angka sensor, tapi status
    -- operasional yang diedit ADMIN langsung di halaman laporan (klik toggle),
    -- bisa lebih dari satu elevasi ON bersamaan di hari yang sama, jadi
    -- masing-masing kolom terpisah (bukan satu kolom "elevasi_aktif"). Baris
    -- cuma dibuat untuk tanggal yang pernah disentuh -- tanggal yang belum
    -- ada barisnya dianggap semua OFF (lihat getKpiKualitasData).
    CREATE TABLE IF NOT EXISTS kualitas_pintu_elevasi (
      tanggal DATE PRIMARY KEY,
      manggar_e3 BOOLEAN,
      manggar_e5 BOOLEAN,
      manggar_e7 BOOLEAN,

...
// Template & penomoran SPD untuk apps/spd-admin.html (halaman khusus admin).
//
...
//
// spd_counter menyimpan nomor BERIKUTNYA per tahun (nomor SPD direset tiap
// ganti tahun). Nomor diambil atomik lewat UPDATE ... RETURNING saat surat
// diunduh, jadi dua admin yang mengunduh bersamaan tidak bisa dapat nomor
// yang sama.
async function ensureSpdTables() {
  ...
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS spd_counter (
      tahun INTEGER PRIMARY KEY,
      next_nomor INTEGER NOT NULL
    );
  `);
  spdInitialized = true;
}
